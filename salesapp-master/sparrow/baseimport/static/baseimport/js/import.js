 sparrow.config(['$routeProvider', '$controllerProvider', function($routeProvider, $controllerProvider) {
  ...
  sparrow.registerCtrl('ImportController', function($scope, $element, model, title, postUrl, descr, id, callback, extraData, close, ModalService, Upload){
      ...
      $scope.onMapDropdownBindFinish = function(colIndex, mappingColumns){
        ...
        for(var i=0; i<mappingColumns.length; i++) {
          ...
          for(var j=0; j<keywords.length; j++) {
            if(dataHeader != '' && keywords[j].toLowerCase().indexOf(dataHeader) != -1) {
              matchFound = true;
              break;
            }
          }
          ...
          if(matchFound) {
            ...
            if (mappingColumns[i].is_required == 'true') { 
              $scope.mappingFieldsData.push({'is_required': true, 'data_type': mappingColumns[i].data_type, 'col_label':mappingColumns[i].col_label, 'col_name': mappingColumns[i].col_name, 'msg': ''});
            }else if (mappingColumns[i].is_required == 'false'){
              $scope.mappingFieldsData.push({'is_required': false, 'data_type': mappingColumns[i].data_type, 'col_label':mappingColumns[i].col_label, 'col_name': mappingColumns[i].col_name, 'msg': ''});
            }
            break;
          }
          else if(dataHeader == ''){
            ...
            $scope.mappingFieldsData.push({'is_required': false, 'data_type': '', 'col_label':'', 'col_name': '', 'msg': ''});
            break;
          }
        }
        if (!matchFound && dataHeader !='') {
          ...
          if ($scope.lowerCaseFieldNames.indexOf(dataHeader) != -1){ 
            $scope.mappingFieldsData.push({'is_required': false, 'data_type': '', 'col_label':'', 'col_name': dataHeader, 'msg': ''});
          }
          else{
            $scope.mappingFieldsData.push({'is_required': false, 'data_type': '', 'col_label':'', 'col_name': '', 'msg': ''});
          }
      ...
      $scope.setFocus = function(rowIndex, colIndex){
        setTimeout(function(){
          $('#input_cell_'+rowIndex+'_'+colIndex+'').focus();
        },0);
      }
      ...
      $scope.changeValueInExcelData = function(show, rowIndex, colIndex, data){
        if (!show) {
      ...
      $scope.changeDropdownData = function(index, col_name){
        ...
        for(var i=0; i<$scope.selectedMapColumn.length; i++){
          if (colNameIndex > -1 && i != index && $scope.selectedMapColumn[i] == col_name) {
        ...
        $scope.updateMappingFieldsData(index, is_required, data_type, col_label, col_name);
      }

      $scope.updateMappingFieldsData = function(index, is_required, data_type, col_label, col_name){
      ...
      $scope.isValidData = function(colIndex, data, rowIndex, show){
        ...
        var fromRequired = 0;
        data = data.toString();
        if(rowIndex != 0 && $scope.mappingFieldsData[colIndex].is_required && $scope.mappingFieldsData[colIndex].data_type == 'float' && data == ''){
          $scope.excelData[rowIndex][colIndex] = 0;
          return false;
        }
        if (rowIndex != 0 && $scope.mappingFieldsData[colIndex].is_required && data == ''){
          $scope.mappingFieldsData[colIndex].msg = "Required data cannot be blank.";
          $scope.headerWarning[colIndex] = true;
          fromRequired = 1;
        }else if ($scope.mappingFieldsData[colIndex].is_required && data != '') {
          if (show) {
          ...
          switch($scope.mappingFieldsData[colIndex].data_type){
            case 'int':
              if (!(Math.floor(data) == +data && $.isNumeric(data))) {
                $scope.headerWarning[colIndex]
                $scope.mappingFieldsData[colIndex].msg = "Numeric data is required.";
                $scope.headerWarning[colIndex] = true;
                return true;
              }else{
                if (show) {
            ...
            case 'date':
              if (!(isValidDate(data))) {
                $scope.mappingFieldsData[colIndex].msg = "Please add date in yyyy-mm-dd format.";
                $scope.headerWarning[colIndex] = true;
                return true;
              }else{
                if (show) {
              ...
              break;
            case 'bool':
              if (!($.trim(data).toLowerCase() == 'true' || $.trim(data).toLowerCase() == 'false' || $.trim(data).toLowerCase() == '0' || $.trim(data).toLowerCase() == '1' || $.trim(data).toLowerCase() == 'yes' || $.trim(data).toLowerCase() == 'no')) {
                $scope.mappingFieldsData[colIndex].msg = 'Boolean data is required.(Example: yes, no or true, false.)';
                $scope.headerWarning[colIndex] = true;
                return true;
              }else{
                if (show) {
      ...
      $scope.sample = function(){
        ...
        setAutoLookup('id_type_id','/b/lookups/product_type/', '', '', false, true);
        $('#hid_type_id').val(' ');
      }

      $scope.sampleProduct = function(event){
        var groupId = $('#hid_type_id').val() != undefined ? $('#hid_type_id').val() : 0;
        $('#idProductImport').modal('hide');
        $('#id_type_id').val(' ');
        window.open("/baseimport/export_sample_product/"+groupId+'/','_blank');
      }
      ...
      $scope.uploadfile = function() {
        ...
        if($('#data_file').val()!='' && !($('#data_file').val().split('.').pop() == 'csv' || $('#data_file').val().split('.').pop() == 'xls' || $('#data_file').val().split('.').pop() == 'xlsx' )){
          if($('#data_file-error').length > 0){
            $('#data_file-error').text('Please uplaod csv, xls only.');
          }else{
            $('#data_file').after('<span id="data_file-error" class="help-block" style="display: block;">Please uplaod csv, xls only.</span>');
          }
        ...
        sparrow.postForm(postData, $('#frmImport'), $scope, function(result) {
          $scope.$applyAsync(function() {
              ...
              if(dropdown_data != undefined && dropdown_data != null){
                for(i = 0; i<dropdown_data.length; i++){
                  $(".selectList").append(new Option(dropdown_data[i].col_label, dropdown_data[i].col_name));
                }
              }
              ...
              if(data != undefined && data!=null){
                ...
                for(var i = 0; i<$scope.fields.length; i++){
                  $scope.lowerCaseFieldNames.push($scope.fields[i].toLowerCase());
                }

                $scope.mappingColumns = result['mapping_columns'];
                $('#is-first-row').parent().parent().remove();

                if(result.file_type != 'csv' && $('#delimiter-parent').length == 1){
                  $('#delimiter-body').empty();
                }

                if(result.file_type == 'csv' && $('#delimiter-parent').length == 0){
                  $('#delimiter-body').append('<h4 style="margin: 0;">Choose delimiters your data contains:</h4><div id="delimiter-parent" class="row" style="margin-bottom:10px;"><div class="col-sm-12" style="margin-left:10px;"><label class="radio-inline"><input type="radio" name="delimiter" value=",">Comma</label><label class="radio-inline"><input type="radio" name="delimiter" value=";">Semicolon</label><label class="radio-inline"><input type="radio" name="delimiter" value="|">Pipe</label><label class="radio-inline"><input type="radio" name="delimiter" value="\t">Tab</label></div></div>');
                }

                if(result.file_type == 'csv' && $('#delimiter-parent').length == 1){
                  $("input[name=delimiter][value='" + result['delimiter'] + "']").attr('checked', 'checked');
                }
                ...
                for(var i=0; i<$scope.mappingColumns.length; i++){
                  $scope.isFieldMapped[$scope.mappingColumns[i].col_name] = false;
                  if ($scope.mappingColumns[i].is_required == 'true') {
                    $scope.required_fields.push($scope.mappingColumns[i].col_name);
                  }
                  $scope.mappingFields[$scope.mappingColumns[i].col_name] = {'is_required': $scope.mappingColumns[i].is_required, 'data_type': $scope.mappingColumns[i].data_type, 'col_label':$scope.mappingColumns[i].col_label}
                }

                var marginClass = '';
                if(model_name == 'bom_line'){
                  $('#delimiter-body').append('<div style="margin-top:-10px;margin-bottom:5px;"><label><label style="margin-right:10px; font-size: 16px;">BOM Name</label><input type="text" style="width: 200px;" autofocus id="id_bom_name" name="bom_name" value="'+extraData['product_name']+'"></label></div>');
                  marginClass = 'style="margin-left: 91px;"';
                }
                $('#delimiter-body').append('<div style="margin-top:0px;margin-bottom:5px;"><label '+marginClass+'><input type="checkbox" id="is-first-row" checked="checked"><label style="position:relative;top:-2px;left:5px;">First row contains column header</label></label></div>');
                if( model_name == 'purchase_order_line' || model_name == 'sale_order_line' || model_name == 'mfg_bom_line' || model_name == 'bom_line'){
                  $('#delimiter-body').append('<div style="margin-top:-10px;margin-bottom:5px;"><label '+marginClass+'><input type="checkbox" id="is-create-product" checked="checked"><label style="position:relative;top:-2px;left:5px;">Automatically create product if not exist</label></label></div>');
                }
      ...
      $scope.uploadImageDailouge = function(files){
        ...
        for(var i= 0 ; i < files.length; i++){
            var file_name = files[i].name.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-');
            $scope.files[i]['objectName'] =file_name;             
        }
        if(files.length > 0 ){
          $scope.imageUploadBtn = true;
          $('#btnUploadImageFolder').show();
          var fileStatus = (/(progress|succeess|failed)$/i).test($scope.files[0]['status']); 
          if(fileStatus == true){
      ...
      $scope.uploadImageFolder = function(event){
          ...
          if(files.length == 0){
              sparrow.showMessage("appMsg", sparrow.MsgType.Error, 'Please drag and drop image folders to upload.', 10);
              return;
          }
          if(files.length > 500){
              sparrow.showMessage("appMsg", sparrow.MsgType.Error, 'You can upload maximum 500 documents at a time.', 10);
              return;
          }
          ...
          $scope.startUploadImageFolder = function(file, totalFiles) {
              Upload.upload({
                  ...
                  data: {file: file, path:file['path'], fileIndex: counter}
              }).then(function (resp) {
                 var file_name = resp.data.file_name.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-');
        ...
        for(var i = 0; i<$scope.excelData.length; i++){
          ...
          for(var j = 0; j < $scope.excelData[i].length; j++){
            ...
            if (data_type == 'bool' && $scope.excelData[i][j] != '') {
              ...
              else if ($.trim($scope.excelData[i][j]).toLowerCase() == 'no') {
                $scope.excelData[i][j] = '0';
              }
            }
            rowData[$scope.mappingFieldsData[j].col_name] = $scope.excelData[i][j].toString();
          }
      ...
      $scope.send_data_to_server = function(url, radio_value, dropdown_Value, createProduct, bom_name){
        allData = $scope.excel_data_batch();
        sparrow.post(url, {id:id,radio_value:radio_value,dropdown_Value:dropdown_Value,createProduct:createProduct, data:JSON.stringify(allData), bom_name: bom_name}, false, function(data) {
          if(data.code == 1) {
            if (sendData < $scope.excelData.length - 1 ) {
              $scope.$apply(function(){
                if (model_name != 'bom_line') {
                  $scope.excelData.splice(0, 99);
                }
                $scope.send_data_to_server(url, radio_value, dropdown_Value, createProduct);
                var dataSend = Math.floor((100*sendData)/$scope.excelData.length);
                if (dataSend <= 100) {
                ...
                if (key_data.length > 0) {
                  for(var j=0; j<key_data.length; j++){
                    if (!$scope.errorData[key_data[j].index]) {
                      ...
                      $scope.errorData[key_data[j].index].push({'value': key_data[j].value, 'type': $scope.keys[i]});
                    }else{
                      $scope.errorData[key_data[j].index].push({'value': key_data[j].value, 'type': $scope.keys[i]});
                    }
                  }
            ...
            $scope.$apply(function(){
              ...
              $scope.errorMsg = $scope.uploadedDataCount+' data uploaded out of '+$scope.totalExcelData;
      ...
      $scope.importItems = function(check_product){
        ...
        if (model_name == 'bom_line') {
          ...
          if(bom_name == ''){
            sparrow.showMessage("appMsg", sparrow.MsgType.Error, 'Please enter bom name.', 10);
            return false;
          }
        }
        if (check_product) {
        ...
        for(var i=0; i<$scope.required_fields.length; i++ ){
          if ($scope.selectedMapColumn.indexOf($scope.required_fields[i]) == -1) {
            sparrow.showMessage("appMsg", sparrow.MsgType.Error, 'Please map all required fields.', 10);
            return false;
          }
        }
        ...
        if (columns != '') {
          sparrow.showMessage("appMsg", sparrow.MsgType.Error, 'Column(s) '+columns+' contains invalid data.', 10);
          return false;
        }else if(dropdown_Value != null && dropdown_Value != 'none'){
          if ($scope.selectedMapColumn.indexOf(dropdown_Value) == -1) {
            selectedOption = $('.selectList option[value="'+$('.selectList').val()+'"]').text()
            sparrow.showMessage("appMsg", sparrow.MsgType.Error, 'Selected option '+selectedOption+' not mapped.', 10);
            return false;
          }
        }
        ...
        $scope.send_data_to_server(url, radio_value, dropdown_Value, createProduct, bom_name);
