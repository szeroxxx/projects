  ...
  sparrow.registerCtrl(
    ...
    function (
      ...
      $scope.SaveRole = function (event) {
        ...
        var selected = [];
        var selected_role = $("input[type='radio'][name='role_opera_user']:checked").val();
        if(selected_role == "operator"){
            $("input[name=checkoperator]:checked").each(function () {
               selected.push($(this).val());
            });
        }
        else if(selected_role == "customer user"){
      ...
      $(document).ready(function () {
        if (document.myForm.role_opera_user.value == "operator") {
          $("#customer_user").removeClass("perm_section");
          $("#operators").addClass("perm_section");
        }
        if (document.myForm.role_opera_user.value == "customer user") {
          $("#operators").removeClass("perm_section");
          $("#customer_user").addClass("perm_section");
        }
      ...
      var rad = document.myForm.role_opera_user;
      if(rad.value == 'operator'){
        $("#customer_user").removeClass("perm_section");
        $("#operators").addClass("perm_section");
        $(".customer_user").hide();
        $(".operators").show();
      }
      else if(rad.value == 'customer user'){
        $("#operators").removeClass("perm_section");
        $("#customer_user").addClass("perm_section");
          $(".customer_user").show();
          $(".operators").hide();
          userPermission(20, "Dashboard", "Dashboard");
      }
      else {
        $("#customer_user").removeClass("perm_section");
        $("#operators").addClass("perm_section");
          ...
          rad[i].addEventListener('change', function() {
              if(this.value == 'operator'){
                ...
                $("#operators").addClass("perm_section");
                  ...
                  var list = $li.find("ul");
                  list.css("display", "block");
                  userPermission(7, "Design", "Job Processing");
              }
              else{
                $("#operators").removeClass("perm_section");
                $("#customer_user").addClass("perm_section");
      ...
      function userPermission(id, name, parent_name){
      ...
      $scope.permclick = function (id, name, parent_name) {
        if (parent_name == "Customer Reports") {
          var parent_name = "Reports";
        }
        if(rad.value == 'operator'){
          $("#customer_user").removeClass("perm_section");
          $("#operators").addClass("perm_section");
          if(parent_name =="Exceptions" || parent_name =="Documents" || parent_name =="Message" || parent_name == "Bug Report"){
              $(".allcheck").hide();
          }
          else{
              $(".allcheck").show();
        ...
        $("ul").removeClass("onActive");
        $(".single_menu").removeClass("onActive");
        $("#menu_" + id).addClass("onActive").css("display", "block");
      ...
      function mainCheckBoxSelection(id) {
        if (
          $('input[data-id="' + id + '"]:checked').length ==
          $('div[data-id="' + id + '"]').length
        ) {
        ...
        if (detailPageEditMode) {
            $(".head :input").prop('disabled', false);
        } else {
          // $(".head :input").prop('disabled', true);
        }
    }
      ...
      if ($routeParams.id == 0) {
        $("#id_operator").prop("disabled", false)
        $("#id_customer_user").prop("disabled", false)
        $("#selectAllId").prop("disabled", false)
      }
      ...
      $scope.applyEditMode = function (e) {
        ...
        $("#selectAllId").prop("disabled", false)
        var permId = "";
        if ($(".role_menus.onActive").length > 0) {
          permId = $(".role_menus.onActive")[0].id.split("_");
        } else {
          permId = $(".single_menu.onActive")[0].id.split("_");
        }
        $(".permCheckbox").prop("disabled", false);
        sparrow.applyEditMode("#frmRole", "#id_name");
        mainCheckBoxSelection(permId[1]);
      };
