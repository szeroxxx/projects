function filesInit(data) {
  sparrow.registerCtrl(
    ...
    function (
      ...
      $scope.fileDownload = function (uid = uid) {
        window.open(
      ...
      $scope.categoryChange = function (file_type) {
        if (
          file_type == "GERBERFILE" ||
          file_type == "ASSEMBLYSTENCILORDERFILE" ||
          file_type == "ORDERSIFILE" ||
          file_type == "ASIFILE" ||
          file_type == "STENCILPANELFILE" ||
          file_type == "PANELFILE" ||
          file_type == "REMARKFILE"
        ) {
          $("#id_file_type").attr("data-rule-extension", "zip|rar");
          $("#id_file_type").attr(
            "data-msg-extension",
            "Please upload a file with a valid extension .zip or .rar file only."
          );
          $("#id_file_type").attr("name", file_type);
        } else if (file_type == "BOMFILE") {
          $("#id_file_type").attr(
            "data-rule-extension",
          ...
          $("#id_file_type").attr(
            "data-msg-extension",
            "Please upload a file with a valid extension like .xls, .xlsx, .csv, .doc or .docx only."
          );
          $("#id_file_type").attr("name", file_type);
        } else if (file_type == "SCHEMATICFILE") {
          $("#id_file_type").attr(
            "data-rule-extension",
            "doc|pdf|jpg|jpeg|zip|rar"
          );
          $("#id_file_type").attr(
            "data-msg-extension",
            "Please upload a file with a valid extension like .doc, .pdf, .jpg, .jpeg, .zip or .rar only."
          );
          $("#id_file_type").attr("name", file_type);
        } else if (file_type == "PICKANDPLACE") {
          $("#id_file_type").attr("data-rule-extension", "xls|xlsx|csv|txt");
          $("#id_file_type").attr(
            "data-msg-extension",
            "Please upload a file with a valid extension like  .xls, .xlsx, .csv or .txt only."
          );
          $("#id_file_type").attr("name", file_type);
        } else if (file_type == "EXCEPTION" || file_type == "EXCEPTIONREPLY") {
          $("#id_file_type").attr("data-rule-extension", "pdf|doc|zip|rar");
          $("#id_file_type").attr(
            "data-msg-extension",
            "Please upload a file with a valid extension like  .pdf, .doc, .zip or .rar only."
          );
          $("#id_file_type").attr("name", file_type);
        }
        setFileName($scope.order_from, file_type);
      };
