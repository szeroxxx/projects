...
from qualityapp.models import (CompanyParameter, Layer, Order, OrderFlowMapping,
...
# class qualityappEcPyService(object):
...
#                 response = qualityapp_service.get_ecc(import_order, "/ecpy/qualityapp/order_from_ecc")
...
#                             if order_exception:
#                                 log_views.insert("qualityapp", "order", [exist_order_id], action, user_id, c_ip, "Exception resolved.")
#                             log_views.insert("qualityapp", "order", [exist_order_id], action, user_id, c_ip, "Import order has been updated.")
#                             is_imported = True
#                         else:
...
#                             else:
#                                 user_id = settings.ADMIN_USER
#                             log_views.insert("qualityapp", "order", [odr.id], action, user_id, c_ip, "Order has been created")
...
#                         send_mail(True, "public",[settings.IMPORT_FAILED_EMAIL], "qualityapp 2.0 Import order(s)", f"{faild_order_nr}- EC order import failed.", "", "")
#                         manager.create_from_exception(e)
#                 OrderTechParameter.objects.bulk_create(prepared_order_tech_data)
#                 qualityapp_service.post_ecc(import_orders, "/ecpy/qualityapp/remove_qualityapp_queue_data")
#                 return is_imported
#             else:
#                 return is_imported
#         except Exception as e:
...
#                         is_imported = False
#                         faild_order_nr = order["OrderNumber"]
#                         send_mail(True, "public",[settings.IMPORT_FAILED_EMAIL], "qualityapp 2.0 Import order(s)", f"{faild_order_nr}- Power order import failed.", "", "")
#                         manager.create_from_exception(e)
#                     url = settings.PPM_URL + "/qualityappAPI/DeleteOrder"
...
#             url = settings.PPM_URL + f"/qualityappAPI/GetOrders?type=inq&OrderNr={customer_inquiry_no}"
#             post_data = ""
#             headers = {"Key": settings.PPM_KEY}
#             ec_res = requests.get(url, data=json.dumps(post_data), headers=headers, timeout=5)
#             response = None
...
#                         is_imported = False
#                         faild_order_nr = order["OrderNumber"]
#                         send_mail(True, "public",[settings.IMPORT_FAILED_EMAIL], "qualityapp 2.0 Import order(s)", f"{faild_order_nr}- Power inquiry import failed.", "", "")
#                         manager.create_from_exception(e)
#                     url = settings.PPM_URL + "/qualityappAPI/DeleteOrder"
#                     PPM_KEY = settings.PPM_KEY
#                     headers = {"Key": PPM_KEY, "content-type": "application/json"}
...
#                 qualityapp_service = qualityappEcPyService()
#                 response = qualityapp_service.post_ecc(import_orders, "/ecpy/qualityapp/export_inq_from_ecc")
#                 services = Service.objects.values("id", "name")
#                 prepared_order_tech_data = []
#                 orders_id = []
#                 board_thickness_name = [
...
#                         is_imported = False
#                         faild_order_nr = order["OrderNumber"]
#                         send_mail(True, "public",[settings.IMPORT_FAILED_EMAIL], "qualityapp 2.0 Import order(s)", f"{faild_order_nr}- EC inquiry import failed.", "", "")
#                         manager.create_from_exception(e)
#                 qualityapp_service.post_ecc(orders_id, "/ecpy/qualityapp/remove_ec_inq_queue_data")
#                 OrderTechParameter.objects.bulk_create(prepared_order_tech_data)
#                 return is_imported
#             else:
#                 return is_imported
