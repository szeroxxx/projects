...
from qualityapp.forms import CompanyForm, CompanyParameterForm
...
#                         UserEfficiencyLog)
# from qualityapp.service import ImportOrder, qualityappEcPyService
...
@check_view_permission([{"customer": "qualityapp_customers"}])
def customers_view(request):
    try:
        ...
        return render(request, "qualityapp/customers.html", {"permissions": json.dumps(permissions)})
...
def customer(request, company_id):
    ...
    return render(request, "qualityapp/customer.html", {"form": company_parameter_form, "comp_form": company_form, "company": company})
            ...
            if request.method == "POST":
                ...
                if company_form.is_valid() and company_parameter_form.is_valid():
                    ...
                    if Company.objects.filter(Q(name__iexact=name, is_deleted=False), ~Q(id=company_parameter.company_id)):
                        ...
                        log_views.insert("qualityapp", "company", [company_id], action, request.user.id, c_ip, "Customer details update.")
            ...
            if company_form.is_valid():
                ...
                if not Company.objects.filter(name__iexact=name, is_deleted=False):
                    ...
                    if company_parameter_form.is_valid():
                        ...
                        log_views.insert("qualityapp", "company", [company_instance.id], action, request.user.id, c_ip, "Customer has been created.")
...
def delete_customer(request):
    try:
        if Util.has_perm("delete_customers", request.user) is False:
            ...
            log_views.insert("qualityapp", "company", company_ids, action, request.user.id, c_ip, "Customer has been deleted.")
...
def order_process_flow(request):
    try:
        return render(request, "qualityapp/order_process_flow.html")
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def order_flow(request, id):
    try:
        ...
        processes = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"])).values("id", "name", "sequence", "code").order_by("sequence")
        ...
        return render(request, "qualityapp/order_flow.html", {"services": services, "processes": processes, "services_processes": services_processes})
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def place_order(request):
    return render(request, "qualityapp/place_order.html")


@check_view_permission([{"order_allocations": "qualityapp_order_allocation"}])
def order_allocations(request):
    try:
        ...
        return render(request, "qualityapp/order_allocations.html", {"manage_auto_allocation" : manage_auto_allocation, "u_id": u_id, "permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
@check_view_permission([{"user_efficiencies": "qualityapp_user_efficiency"}])
def user_efficiencies(request):
    try:
        user = User.objects.get(id=request.user.id)
        perms = ["add_new_user_efficiency", "edit_user_efficiency", "can_export_user_efficiency"]
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/user_efficiencies.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def orders(request):
    try:
        ...
        return render(request, "qualityapp/reports/orders.html", con)
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def work_details_reports(request):
    try:
        ...
        return render(request, "qualityapp/reports/work_details_reports.html", context)
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
        ...
        if operator_id:
            if operator_id["operator_type"]:
                if operator_id["operator_type"] == "PLANET_ENG":
                    planet_engineer = "Yes"
        context = {"planet_engineer": planet_engineer, "operator_id": operator_id["id"], "permissions": json.dumps(permissions)}
        return render(request, "qualityapp/reports/work_summary_reports.html", context)
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
@check_view_permission([{"customer_user": "qualityapp_customer_users"}])
def customer_users_view(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/customer_users.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def customer_user(request, user_id):
    ...
    return render(request, "qualityapp/customer_user.html", {"user_role": user_role, "user": user, "comp_user": company_user})
        ...
        if user_id != "0":
            if Util.has_perm("edit_customer_users", request.user) is False:
                ...
                if User.objects.filter(Q(username__iexact=username), ~Q(id=user_id)):
                    ...
                    c_ip = base_views.get_client_ip(request)
                    action = AuditAction.UPDATE
                    log_views.insert("qualityapp", "companyuser", [user_id], action, request.user.id, c_ip, "Customer-user details update.")
            ...
            if Util.has_perm("add_new_customer_users", request.user) is False:
                ...
                if not User.objects.filter(username__iexact=username):
                    ...
                    c_ip = base_views.get_client_ip(request)
                    action = AuditAction.INSERT
                    log_views.insert("qualityapp", "companyuser", [user.id], action, request.user.id, c_ip, "Customer-user has been created.")
...
def customer_user_change_password(request):
    try:
        ...
        c_ip = base_views.get_client_ip(request)
        action = AuditAction.UPDATE
        log_views.insert("qualityapp", "companyuser", [user_id], action, request.user.id, c_ip, "Password updated.")
...
def delete_customer_user(request):
    try:
        if Util.has_perm("delete_customer_users", request.user) is False:
            ...
            c_ip = base_views.get_client_ip(request)
            action = AuditAction.DELETE
            log_views.insert("qualityapp", "companyuser", user_ids, action, request.user.id, c_ip, "Customer-user has been deleted.")
...
def set_order_screen(request, company_id):
    try:
        ...
        return render(request, "qualityapp/set_order_screen.html", {"order_screen": order_screen_params, "company_id": company_id})
    except Exception as e:
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def set_order_screen_copy(request, company_id):
    try:
        ...
        return render(request, "qualityapp/set_order_screen.html", {"order_screen": order_screen_params, "company_id": select_customer})
...
@check_view_permission([{"operator": "qualityapp_operators"}])
def operators(request):
    user = User.objects.get(id=request.user.id)
    perms = ["can_export_operators"]
    permissions = Util.get_permission_role(user, perms)
    return render(request, "qualityapp/operators.html", {"permissions": json.dumps(permissions)})
...
def operator(request, operator_id):
    ...
    dor = ""
    sub_group = None
    subgroup_of_operators = SubGroupOfOperator.objects.values("id", "sub_group_name")
    ...
    if operator_id != "0":
        ...
        if operator.sub_group_of_operator:
            ...
            sub_group = SubGroupOfOperator.objects.filter(id=id_sub_group).values("id", "sub_group_name").first()
    ...
    return render(request, "qualityapp/operator.html", context)
...
def save_operator(request):
    try:
        ...
        if user_sub_group_data:
            is_exist_sub_group = SubGroupOfOperator.objects.filter(sub_group_name=user_sub_group_data).values("id").first()
        ...
        if operator_id != "0":
            if Util.has_perm("edit_operators", request.user) is False:
                ...
                if User.objects.filter(Q(username__iexact=username), ~Q(id=operator_id)):
                    ...
                    c_ip = base_views.get_client_ip(request)
                    action = AuditAction.UPDATE
                    log_views.insert("qualityapp", "operator", [operator_id], action, request.user.id, c_ip, "Operator details update.")
...
def create_sub_group_of_operator(request):
    try:
        ...
        if sub_group:
            is_exist_sub_group = SubGroupOfOperator.objects.filter(sub_group_name=sub_group).values("id").first()
            if is_exist_sub_group:
                response = {"code": 0}
                return HttpResponse(json.dumps(response), content_type="json")
            else:
...
def operator_change_password(request):
    try:
        ...
        c_ip = base_views.get_client_ip(request)
        action = AuditAction.UPDATE
        log_views.insert("qualityapp", "operator", [user_id], action, request.user.id, c_ip, "Password updated.")
        response = {"code": 1, "msg": "Password updated."}
        return HttpResponse(json.dumps(response), content_type="json")
    except Exception as e:
        logging.exception("Something went wrong.")
...
def delete_operators(request):
    try:
        if Util.has_perm("delete_operators", request.user) is False:
            ...
            c_ip = base_views.get_client_ip(request)
            action = AuditAction.DELETE
            log_views.insert("qualityapp", "operator", user_ids, action, request.user.id, c_ip, "Operator has been deleted.")
...
def set_order_screen_params(request):
    try:
        ...
        return render(request, "qualityapp/set_order_screen_params.html", con)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def user_efficiency_log(request, order_id, prep_time_, order_to_status, efficeiency_remark, back_to_previous):
    try:
        ...
        if efficeiency_remark != "":
            ...
            base_views.create_remark("qualityapp", "userefficiencylog", user_effi.id, efficeiency_remark, "", request.user.id, "remarks", "Efficeiency_Remarks", "", "", prep_by, prep_on)
            base_views.create_remark("qualityapp", "order", order_id, efficeiency_remark, "", request.user.id, "remarks", "Efficeiency_Remarks", "", "", prep_by, prep_on)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
            ...
            for cus_order_id in order_list:
                ...
                if isorder:
                    ...
                    if operator["operator"]:
                        ...
                        action = AuditAction.UPDATE
                        log_views.insert("qualityapp", "order", [order_id], action, request_user_id, c_ip, order_release_operator)
                    ...
                    log_views.insert("qualityapp", "order", [order_id], action, request_user_id, c_ip, order_reserve_operator)
                    ...
                    log_views.insert_("qualityapp", "order", [order_id], action, request_user_id, c_ip, order_next_status, order["operator"], prep_time_)
                    log_views.insert("qualityapp", "order", [order_id], action, request_user_id, c_ip, order_reserve_operator)
...
def send_to_next_details(request, order_id):
    try:
        ...
        processes = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"]), id__in=ids).values("code", "sequence", "name").order_by("sequence")
        next_status = ""
        ...
        layer = order["layer"] if order["layer"] != "" and order["layer"] is not None else ""
        efficiency = (
            Efficiency.objects.filter(company_id=order["company__id"], service_id=order["service__id"], process__code=order["order_status"]).values("layer", "multi_layer").first()
        )
        layer_point = ""
        layer_ = ""
        layer_ = layer[0:2]
        ...
        return render(request, "qualityapp/send_to_next_details.html", response)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
    ...
    try:
        with transaction.atomic():
            ...
            for_error_check = [{
                ...
                "operator": order["operator_id__user_id__username"]
            }] # it is check for error log
                ...
                if order_status_upl_panel == "upload_panel" and panel_no:
                    for orderlist_id in orders_list:
                        ...
                        upload_and_save_impersonate(
                            upload_file_data, "qualityapp", "order_attachment", orderlist_id, u_id, c_ip, file_type_id["code"], upload_file_name, order_["customer_order_nr"], ""
                        )
                else:
                    Order_Attachment.objects.filter(object_id=order_id, file_type__code=file_type_id["code"]).update(deleted=True)
                    upload_and_save_impersonate(
                        upload_file_data, "qualityapp", "order_attachment", order_id, u_id, c_ip, file_type_id["code"], upload_file_name, order["customer_order_nr"], ""
                    )
            if remarks != "":
                if order_status_upl_panel == "upload_panel" and panel_no:
                    for orderlist_id in orders_list:
                        ...
                        prep_on = auditlog["action_on"]
                        remark = base_views.create_remark("qualityapp", "order", orderlist_id, remarks, "", request.user.id, "remarks", remarks_type, "", "", prep_by, prep_on)
                    ...
                    prep_by = order["operator__user_id"]
                    prep_on = auditlog["action_on"]
                    remark = base_views.create_remark("qualityapp", "order", order_id, remarks, "", request.user.id, "remarks", remarks_type, "", "", prep_by, prep_on)
                    if attachment is not None:
                        attachment_name = str(attachment)
                        attachment_data = attachment.read()
                        upload_and_save_impersonate(
            ...
            if next_status != "order_finish":
                ...
                log_views.insert_("qualityapp", "order", [order_id], action, request.user.id, c_ip, order_next_status, order["operator"], prep_time_)
        ...
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        manager.create_from_text(for_error_check)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def order_finish_mail(request, order_id, order_status_f, prep_time_):
    try:
        with transaction.atomic():
            ...
            action = AuditAction.UPDATE
            log_views.insert_("qualityapp", "order", [order_id], action, request_user_id, c_ip, order_finished, order["operator"], prep_time_)
            log_views.insert("qualityapp", "Order", [order_id], action, request_user_id, c_ip, "Order has been finished")
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
            ...
            if skill_matrix and stop_auto_allocation is False:
                if skill_matrix["operator_ids"] is not None:
                    ...
                    if len(listss) != 0:
                        ...
                        order_reserve_operator = "Operator" + " " + "<b>" + " " + ids["user__username"] + " " + "</b>" + " " + "is reserved"
                        log_views.insert("qualityapp", "order", [order_id], action, user_id, c_ip, order_reserve_operator)
                        is_operator_login = OperatorLogs.objects.filter(operator_id_id=ids["id"], logged_in_time__isnull=False).values("logged_in_time").first()
                        operator_login_time = None
                        if is_operator_login:
                            operator_login_time = is_operator_login["logged_in_time"]
...
def back_to_previous_details(request, order_id):
    try:
        ...
        layer = order["layer"] if order["layer"] != "" and order["layer"] is not None else ""
        efficiency = (
            Efficiency.objects.filter(company_id=order["company__id"], service_id=order["service__id"], process__code="back_to_previous").values("layer", "multi_layer").first()
        )
        layer_point = ""
        layer_ = ""
        layer_ = layer[0:2]
        ...
        return render(request, "qualityapp/back_to_previous_details.html", response)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
        ...
        with transaction.atomic():
            ...
            if remarks_back_to_previous != "":
                ...
                remark = base_views.create_remark(
                    "qualityapp", "order", order_id, remarks_back_to_previous, "", request.user.id, "remarks", remarks_type_back_to_previous, "", "", prep_by, prep_on
                )
            ...
            log_views.insert_("qualityapp", "order", [order_id], action, request.user.id, c_ip, order_previous_status, order["operator"], prep_time_back_to_previous)
...
def order(request, order_id, remarks_status):
    try:
        ...
        return render(request, "qualityapp/order.html", context)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def workspace_search(request, status, id, type):
    try:
        ...
        status = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"]), code__in=order_status_code).values("code", "name")
    ...
    try:
        with transaction.atomic():
            ...
            if upload_image is not None:
                ...
                upload_and_save_impersonate(
                    upload_image_data, "qualityapp", "order_attachment", order_id, request.user.id, c_ip, "EXCEPTION", upload_image_name, order_status["customer_order_nr"], ""
                )
            ...
            if si_file is not None:
                ...
                upload_and_save_impersonate(si_file_data, "qualityapp", "order_attachment", order_id, request.user.id, c_ip, "SI", si_file_name, order_status["customer_order_nr"], "")
            else:
                is_si_file = False
            ...
            log_views.insert("qualityapp", "OrderException", [order_exception.id], action, request.user.id, c_ip, "Exception generated on order")
            log_views.insert("qualityapp", "order", [order_id], action, request.user.id, c_ip, "Exception generated on order")
            ...
            cc_mail = user_cc_mail["user__email"] if user_cc_mail else ""
            email_id = CompanyParameter.objects.filter(company=order_detail["order__company"]).values("ord_exc_gen_mail", "gen_mail", "mail_from").first()
            mail_from = email_id["mail_from"] if email_id["mail_from"] else None
...
def incomings_view(request, type):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/incomings.html", {"type": type, "permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def exceptions_search(request, status, id, type):
    try:
        ...
        if request.POST.get("qualityapp_id"):
            query.add(Q(order__order_number__icontains=request.POST["qualityapp_id"]), query.connector)
...
def put_to_customer(request):
    try:
        ...
        c_ip = base_views.get_client_ip(request)
        action = AuditAction.UPDATE
        log_views.insert("qualityapp", "OrderException", [order_exception_id], action, request.user.id, c_ip, "Exception sent to Customer.")
        log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, "Exception sent to Customer.")
...
def back_to_in_coming(request):
    try:
        ...
        c_ip = base_views.get_client_ip(request)
        action = AuditAction.UPDATE
        log_views.insert("qualityapp", "OrderException", [order_exception_id], action, request.user.id, c_ip, "Exception Order back to the in-coming stage.")
        log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, "Exception Order back to the in-coming stage.")
...
def send_reminder(request):
    try:
        ...
        if order_detail["pre_define_problem__code"] == "Internal Exception":
            ...
            title = "Dear Concern,"
            title_ = "Please check on below details and suggest us back asap : #" + str(order_detail["order__customer_order_nr"]) + "."
        else:
        ...
        if email_id_ or order_detail["pre_define_problem__code"] == "Internal Exception":
            ...
            message = render_to_string(
                "qualityapp/mail_order.html",
                {
                    "mail_type": "internal_exception" if order_detail["pre_define_problem__code"] == "Internal Exception" else None,
                    "internal_remark": internal_remark,
                    "head": head,
                    "title": title,
                    "title_": title_ if order_detail["pre_define_problem__code"] == "Internal Exception" else None,
                    "layers": layers,
                    "order_detail": order_detail,
                ...
                if message_['mail_messages']:
                    if message_['mail_messages']['exceptionId'] == int(order_exception_id):
                        if message_['mail_messages']['exceptionProb'] == "Internal Exception":
                            ...
                            subject = message_['mail_messages']['subject']
        ...
        c_ip = base_views.get_client_ip(request)
        action = AuditAction.UPDATE
        log_views.insert("qualityapp", "OrderException", [order_exception_id], action, request.user.id, c_ip, "Exception reminder sent")
        log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, "Exception reminder sent")
...
def modify_exception(request):
    try:
        ...
        return render(
            request,
            "qualityapp/modify_exception.html",
            {
...
def modify_exception_save(request):
    try:
        with transaction.atomic():
            ...
            log_views.insert("qualityapp", "Order", [str(order_id)], action, request.user.id, c_ip, "Order exception has been updated.")
            log_views.insert("qualityapp", "OrderException", [order_exception], action, request.user.id, c_ip, "Order exception has been updated.")
...
def exception_details(request):
    try:
        ...
        return render(
            request,
            "qualityapp/exception_details.html",
            {
                "order_exception": order_exception,
                "upload_image": exception_files[0],
                "si_file": exception_files[1],
...
def order_remark_save(request):
    try:
        ...
        if order_id is not None:
            ...
            base_views.create_remark("qualityapp", "order", order_id, remarks_back, "", request.user.id, model_remark_field, remarks_type_back, "", "")
        log_views.insert("qualityapp", "order", [order_id], action, request.user.id, c_ip, "Exception order sent back to process")
        log_views.insert("qualityapp", "orderexception", [order_exception], action, request.user.id, c_ip, "Exception order sent back to process")
...
def engineers_work_report_view(request):
    try:
        ...
        return render(request, "qualityapp/reports/engineers_work_report.html", context)
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def compare_orders(request, type):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/compare_orders.html", {"type": type, "permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def compare_orders_search(request, status, id, type):
    try:
        ...
        if status == "ec_pending":
            qualityapp_service = qualityappEcPyService()
            ec_response = qualityapp_service.get_ecc({}, "/ecpy/qualityapp/import_pending_orders_from_ec")[0:20]
            if customer_order_no:
                ec_response = qualityapp_service.get_ecc({"customer_order_nr": customer_order_no}, "/ecpy/qualityapp/import_pending_orders_from_ec")
            ...
            url = settings.PPM_URL + "/qualityappAPI/GetOrders?type=orderall"
            if customer_order_no:
                url = settings.PPM_URL + f"/qualityappAPI/GetOrders?type=order&OrderNr={customer_order_no}"
            post_data = ""
            ...
            url = settings.PPM_URL + "/qualityappAPI/GetOrders?type=inqall"
            if customer_order_no:
                url = settings.PPM_URL + f"/qualityappAPI/GetOrders?type=inq&OrderNr={customer_order_no}"
            post_data = ""
            headers = {"Key": settings.PPM_KEY}
            ec_res = requests.get(url, data=json.dumps(post_data), headers=headers, timeout=5)
            response = {"draw": request.POST["draw"], "data": []}
            ...
            qualityapp_service = qualityappEcPyService()
            ec_response = qualityapp_service.get_ecc({}, "/ecpy/qualityapp/import_inq_from_ecc")[0:20]
            if customer_order_no:
                ec_response = qualityapp_service.get_ecc({"customer_order_nr": customer_order_no}, "/ecpy/qualityapp/import_inq_from_ecc")
            ...
            ec_compare_order = (
                CompareData.objects.filter(query_).values("id", "number", "order_status", "import_from", "compared_on").order_by(sort_col)[start : (start + length)]
            )
            if customer_order_no:
            ...
            for order in ec_compare_order:
                order_sta = {
                    "qualityapp_status" : str(ord["order_status"]) for ord in order_data if str(ord["customer_order_nr"]) == str(order["number"]) and ord["order_status"] is not None
                }
                response["data"].append(
                    {
                        "id" : index,
                        ...
                        "qualityapp_status" : dict(order_status)[order_sta["qualityapp_status"]] if bool(order_sta) is True and order_sta["qualityapp_status"] in dict(order_status) else "",
                    }
                )
                index += 1
            return HttpResponse(AppResponse.get(response), content_type="json")
...
@check_view_permission([{"nc_details": "qualityapp_detail_master"}])
def nc_details(request):
    try:
        user = User.objects.get(id=request.user.id)
        perms = ["add_new_nc_detail", "edit_nc_detail", "can_export_nc_details_master"]
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/nc_details.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def user_efficiencies_delete(request):
    try:
        if Util.has_perm("delete_user_efficiency", request.user) is False:
            ...
            c_ip = base_views.get_client_ip(request)
            action = AuditAction.DELETE
            log_views.insert("qualityapp", "Efficiency", user_efficiencies_ids.split(","), action, request.user.id, c_ip, "User efficiency has been deleted")
...
def user_efficiency(request):
    ...
    return render(request, "qualityapp/user_efficiency.html", con)


def import_order_view(request):
    return render(request, "qualityapp/import_order.html")


def batch_code_view(request):
    return render(request, "qualityapp/batch_code.html")
...
def nc_detail(request):
    try:
        ...
        return render(request, "qualityapp/nc_detail.html", con)
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
    ...
    try:
        if Util.has_perm("delete_nc_detail", request.user) is False:
            ...
            try:
                ...
                c_ip = base_views.get_client_ip(request)
                action = AuditAction.DELETE
                log_views.insert("qualityapp", "NcCategory", nc_details_ids.split(","), action, request.user.id, c_ip, "NC Details has been deleted")
    ...
    try:
        with transaction.atomic():
            ...
            if status == "finished":
                ...
                send_mail(True, "public", [*email_id], subject, message, "", [cc_mail], mail_from)
                log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, history_status)
                log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, "Order has been finished")
            ...
            if operator["operator"]:
                ...
                Order.objects.filter(id=order_id).update(operator=None)
                action = AuditAction.UPDATE
                log_views.insert("qualityapp", "order", [order_id], action, request.user.id, c_ip, order_release_operator)
                reserved_order = ActiveOperators.objects.filter(reserved_order_id_id=order_id).values("logged_in_time").first()
                login_time = reserved_order["logged_in_time"]
                ActiveOperators.objects.filter(reserved_order_id_id=order_id).delete()
                is_active_operator = ActiveOperators.objects.filter(operator_id_id=operator["operator_id"])
            ...
            if status == "cancel":
                ...
                send_mail(True, "public", [*email_id], subject, message, "", [cc_mail], mail_from)
                log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, history_status)
                log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, "Order has been Cancelled")
                response = {"code": 1, "msg": "Order status update."}
...
def nc_report(request):
    try:
        ...
        return render(request, "qualityapp/reports/nc_report.html", con)
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
            ...
            if auditlogs:
                ...
                if nc_report_ids:
                    ...
                    if file is not None:
                        file_name = str(file)
                        file_data = file.read()
                        upload_and_save_impersonate(file_data, "qualityapp", "order_attachment", order, request.user.id, c_ip, "NC_FILE", file_name, order_number, docnumber.nextnum)
                NonConformityDetail.objects.bulk_create(nc_report_details_list)
                log_views.insert("qualityapp", "nonconformity", nc_report_ids_list, action, request.user.id, c_ip, "NC report has been created")
                log_views.insert("qualityapp", "Order", [order], action, request.user.id, c_ip, "NC report has been created")
                docnumber.increase()
                docnumber.save()
            request_user = request.user.id
            request_user_id = User.objects.filter(id=request_user).values("id", "username").first()
...
def change_order_status(request):
    try:
        ...
        if id != "0":
            ...
            order_flow_mapping = OrderFlowMapping.objects.filter(company_id=order["company__id"], service_id=order["service__id"]).values("process_ids").first()
            ids = order_flow_mapping["process_ids"].split(",") if order_flow_mapping and order_flow_mapping["process_ids"] is not None else []
            processes = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"]), id__in=ids).values("code", "name").order_by("sequence")
            code = []
            name = []
            for process in processes:
        ...
        return render(request, "qualityapp/reports/order_status.html", con)
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def nc_reports(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/nc_reports.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def order_details_update(request):
    try:
        ...
        log_views.insert("qualityapp", "order", [order_id], action, request.user.id, c_ip, "Order has been updated")
...
def add_remark(request):
    try:
        ...
        model_remark_field = "remarks"
        if order_id is not None:
            base_views.create_remark("qualityapp", "order", order_id, remarks_add, "", request.user.id, model_remark_field, remarks_type_add, "", "")
            action = AuditAction.INSERT
            log_views.insert("qualityapp", "Order", [order_id], action, request.user.id, c_ip, "Remark has been added")
...
def utility_place_order(request, company_id, company_user_id):
    try:
        ...
        return render(
            request,
            "qualityapp/utility_place_order.html",
...
def save_utility_place_order(request, company_id, company_user_id):
    try:
        with transaction.atomic():
            ...
            processes = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"]), id__in=process_ids).values("code", "sequence").order_by("sequence")
            ...
            if file_ is not None:
                ...
                upload_and_save_impersonate(file_data, "qualityapp", "order_attachment", order.id, request.user.id, c_ip, "ORDERFILE", file_name, order_number, "")
            ...
            if email_id:
                ...
                send_mail(True, "public", [*email_id], subject, message, "", [cc_mail], mail_from)
            log_views.insert("qualityapp", "order", [order.id], action, company_user_id, c_ip, "Order has been created")
            if remarks != "":
                base_views.create_remark("qualityapp", "order", order.id, remarks, "Cus_rema", request.user.id, model_remark_field, "Customer_Remarks", "", "")
            if cam_remark:
                base_views.create_remark("qualityapp", "order", order.id, order_tech["cam_remark"], "Cum_rema", request.user.id, model_remark_field, "Customer_CAM_Remarks", "", "")
...
def work_detail_customer(request):
    ...
    permissions = Util.get_permission_role(user, perms)
    return render(request, "qualityapp/reports/work_detail_customer.html", {"permissions": json.dumps(permissions)})
...
def summary_report_customer(request):
    ...
    permissions = Util.get_permission_role(user, perms)
    return render(request, "qualityapp/reports/summary_report_customer.html", {"permissions": json.dumps(permissions)})
...
def engineers_work_report_process_count(request):
    try:
        ...
        if company_id and service_id:
            ...
            process_id = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"])).values("code")
...
def admin_dashboard(request):
    return render(request, "qualityapp/dashboard.html")
...
def logged_in_user(request):
    ...
    permissions = Util.get_permission_role(user, perms)
    return render(request, "qualityapp/logged_in_user.html", {"permissions": json.dumps(permissions)})
...
def save_engineers_work_report(request):
    try:
        with transaction.atomic():
            ...
            company_id = request.POST.get("company_id")
            service_id = request.POST.get("service_id")
            process_id = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"])).values("id", "code")
...
def send_exception_mail(request):
    try:
        if request.method == "POST":
            ...
            email_id = [email_ids for email_ids in email_id.split(",")]
            order_detail = OrderException.objects.filter(order=order_id).values("order__company", "pre_define_problem__code").first()
            company_gen_mail = (
                CompanyParameter.objects.filter(company=order_detail["order__company"])
            ...
            query_result.append(
                {
                    ...
                    "qualityapp_id": operator["reserved_order_id__order_number"],
...
def skill_matrixs(request):
    return render(request, "qualityapp/skill_matrixs.html")
...
def skill_matrix(request, company_id):
    try:
        ...
        if exiest is False:
            process_id = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"])).values("id")
            skill_matrix = []
            for process_id_ in process_id:
        ...
        return render(request, "qualityapp/skill_matrix.html", context)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
# def import_orders_and_inquiries(request):
    try:
        ...
        if order_type == "ECORD":
            response = qualityapp_service.get_ecc([{"order_type": "ECORD"}], "/ecpy/qualityapp/orders_and_inquiries_export")
        ...
        if order_type == "ECINQ":
            response = qualityapp_service.get_ecc([{"order_type": "ECINQ"}], "/ecpy/qualityapp/orders_and_inquiries_export")
            customer_orders = [order_nr["Number"] for order_nr in response]
...
def orders_in_flow(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/orders_in_flow.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def exports_orders(request):
    try:
        ...
        if request.POST.get("pcb_name"):
            query.add(Q(pcb_name__icontains=request.POST["pcb_name"]), query.connector)
        if request.POST.get("qualityapp_id"):
            query.add(Q(order_number__icontains=request.POST["qualityapp_id"]), query.connector)
        if request.POST.get("customer_order_nr"):
            query.add(Q(customer_order_nr__icontains=request.POST["customer_order_nr"]), query.connector)
        if request.POST.get("company"):
            query.add(Q(company__name__icontains=request.POST["company"]), query.connector)
...
def exports_order_allocations(request):
    try:
        ...
        if process_id != "":
            process = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous"]), id=process_id).values("code").first()
...
def live_prep_tracking_report(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/live_prep_tracking_report.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def user_efficiency_report(request):
    try:
        ...
        return render(request, "qualityapp/reports/user_efficiency_report.html", {"today": today, "permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("something went wrong")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
                    ...
                    response["data"].append(
                        {
                            ...
                            "pi": Util.decimal_to_str(request, int(userefficiencylog["total_efficiency"]) / total_days) if userefficiencylog["total_efficiency"] else 0,
...
def compare_order_and_inq(request):
    try:
        ...
        if compare_type == "ECORDERS":
            qualityapp_service = qualityappEcPyService()
            ec_response = qualityapp_service.get_ecc({}, "/ecpy/qualityapp/get_orders_for_qualityapp_compare")
        ...
        if compare_type == "ECINQ":
            qualityapp_service = qualityappEcPyService()
            ec_response = qualityapp_service.get_ecc({}, "/ecpy/qualityapp/get_inq_for_qualityapp_compare")
        ...
        if compare_type == "POWERORDERS":
            url = settings.PPM_URL + "/qualityappAPI/GetOrders?type=OrderTracker"
            post_data = ""
            headers = {"Key": settings.PPM_KEY}
        ...
        if compare_type == "POWERINQUIRY":
            url = settings.PPM_URL + "/qualityappAPI/GetOrders?type=InquiryTracker"
            post_data = ""
            headers = {"Key": settings.PPM_KEY}
            power_res = requests.get(url, data=json.dumps(post_data), headers=headers, timeout=5)
            data = None
...
@check_view_permission([{"performance_index": "qualityapp_performance_index"}])
def performance_indexes(request):
    try:
        user = User.objects.get(id=request.user.id)
        perms = ["can_add_performance_index", "can_update_performance_index", "can_delete_performance_index"]
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/performance_indexes.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def performance_index(request):
    try:
        ...
        return render(request, "qualityapp/performance_index.html", {"performance_indexes": performance_indexes})
    except Exception as e:
        manager.create_from_exception(e)
        logging.exception("Something went wrong.")
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
    ...
    try:
        with transaction.atomic():
            if Util.has_perm("can_delete_performance_index", request.user) is False:
                ...
                c_ip = base_views.get_client_ip(request)
                action = AuditAction.DELETE
                log_views.insert("qualityapp", "PerformanceIndex", performance_index_ids.split(","), action, request.user.id, c_ip, "Performance index has been deleted.")
...
def modify_nc(request):
    try:
        ...
        return render(request, "qualityapp/reports/modify_nc.html", con)
    except Exception as e:
        manager.create_from_exception(e)
        logging.exception("Something went wrong.")
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
        ...
        with transaction.atomic():
            ...
            if auditlogs:
                ...
                if nc_file is not None:
                    ...
                    upload_and_save_impersonate(nc_file_data, "qualityapp", "order_attachment", order_id, request.user.id, c_ip, "NC_FILE", nc_file_name, order_number, nc_number)
                if car_file is not None:
                    car_file_name = str(car_file)
                    car_file_data = car_file.read()
                    upload_and_save_impersonate(car_file_data, "qualityapp", "order_attachment", order_id, request.user.id, c_ip, "CAR_FILE", car_file_name, order_number, nc_number)
                action = AuditAction.UPDATE
                log_views.insert("qualityapp", "nonconformity", [nc_id], action, request.user.id, c_ip, "NC report updated.")
...
def orders_with_qta(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/orders_with_qta_reports.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong!")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def remark_report(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/remark_report.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
        ...
        if "load_data" in request.POST:
            ...
            for remark in all_remarks:
                response["data"].append(
                    {
                        "id": remark["id"],
                        "company__name": company__name[remark["entity_id"]],
                        "customer_order_nr": customer_order_nr[remark["entity_id"]],
                        "order_number": order_number[remark["entity_id"]],
                        "layer": layers_[layer[remark["entity_id"]] if remark["entity_id"] in layer else ""] if layer[remark["entity_id"]] in layers_ else "",
                        "service__name": service__name[remark["entity_id"]],
            ...
            sort_col_asc = ["company__name", "customer_order_nr", "order_number", "service__name"]
            sort_col_desc = ["-company__name", "-customer_order_nr", "-order_number", "-service__name"]
            if sort_col in sort_col_desc:
                response["data"] = sorted(response["data"], key=lambda i: i[sort_col[1:]], reverse=True)
            if sort_col in sort_col_asc:
                response["data"] = sorted(response["data"], key=lambda i: i[sort_col])
...
def user_efficiency_add_remark(request, operator_id, select_date):
    try:
        ...
        return render(request, "qualityapp/reports/user_efficiency_add_remark.html", context)
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
...
def update_user_efficiency_report(request):
    try:
        with transaction.atomic():
            ...
            action = AuditAction.UPDATE
            c_ip = base_views.get_client_ip(request)
            log_views.insert("qualityapp", "userefficiencylog", user_efficiency_id_list, action, request.user.id, c_ip, "Minimum efficiency and Target efficiency has been updated.")
...
def exceptions_report(request):
    try:
        ...
        permissions = Util.get_permission_role(user, perms)
        return render(request, "qualityapp/reports/exceptions_report.html", {"permissions": json.dumps(permissions)})
    except Exception as e:
        logging.exception("Something went wrong.")
        manager.create_from_exception(e)
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
    ...
    try:
        ...
        if "load_data" in request.POST:
            ...
            order_exceptions = (
                ...
                .annotate(
                    qualityapp_id=F("order__order_number"),
            ...
            for order_exception in order_exceptions:
                response["data"].append(
                    {
                        "id": order_exception["id"],
                        "qualityapp_id": order_exception["qualityapp_id"],
