    ...
    try:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                user = form.save()
                response = {"id": user.id, "code": 1, "msg": "Send mail"}
...
def notifications(request):
    try:
        if request.method == "POST":
            ...
            response = {"notifications": [], "user_notifications": [], "count": count, "user_record": user_record, "general_record": general_record}
            ...
            for user_notification in user_notifications:
                ...
                response["user_notifications"].append(
                    {"id": user_notification.id, "subject": user_notification.subject, "is_read": user_notification.is_read, "created_on": created_on}
                )

            for notification in notifications:
                created_on = Util.get_local_time(notification.created_on, True) if notification.created_on is not None else ""
                response["notifications"].append({"id": notification.id, "subject": notification.subject, "is_read": notification.is_read, "created_on": created_on})
...
def mark_as_all_read(request):
    try:
        with transaction.atomic():
            ...
            if is_user_nots:
                Notification.objects.filter(is_read=False, user_id=user_id).update(is_read=True, read_by_id=request.user.id, read_on=datetime.datetime.utcnow())
            else:
                Notification.objects.filter(is_read=False, user_id__isnull=True).update(is_read=True, read_by_id=request.user.id, read_on=datetime.datetime.utcnow())

            notification_count = Notification.objects.filter(is_read=False, user_id__isnull=True).count()
            user_notification_count = Notification.objects.filter(is_read=False, user_id=request.user.id).count()
            total_count = notification_count + user_notification_count
            response = {"code": 1, "msg": "Data saved.", "notification_count": notification_count, "total_count": total_count}
            return HttpResponse(AppResponse.get(response), content_type="json")
    except Exception as e:
        return HttpResponse(AppResponse.msg(0, str(e)), content_type="json")
