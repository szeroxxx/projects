...
function initSparrow() {
  ...
  setAutoLookup = function (
    ...
    if (storeInstance) {
      var autoSuggests =
        app.global.get(app.global.keys.AUTO_SUGGEST) == undefined
          ? {}
          : app.global.get(app.global.keys.AUTO_SUGGEST);
    ...
    $("#" + id).on("addNewLink", function (e, data) {
      if (data["dataUrl"]) {
        var openIframeDialog = openIframe(
          "/b/iframe_index/" + data["dataUrl"],
          data["title"],
          function (dataObject) {
            var isUpdate = true;
            ...
            if (
              ...
              basename != "" &&
              (($("#h" + basename).val() != undefined &&
                parseInt($("#h" + basename).val()) != dataObject[basename]) ||
                $("#h" + basename).val() == undefined)
            ) {
              isUpdate = false;
            }
            if (dataObject != undefined && isUpdate && dataObject["id"] != 0) {
              ...
              eval(
                $(ms.input).closest("div.form-group").removeClass("has-error")
              );
              $("#appMsg").hide();
            }
          }
  ...
  function mainController($scope, $rootScope, $route, $compile, $location) {
    ...
    $scope.$on(
      ...
      function (event, searchParameter, tableIndex, cacheParam) {
        if (tableIndex != 98) {
          searchParts[searchParameter.name] = searchParameter.value;
          $rootScope.searchParts = searchParts;
          return;
        }
        ...
        $scope["searchParams" + tableIndex][searchParameter.key] =
          searchParameter.value;
        $rootScope.searchFor =
          searchParameter.name + " " + searchParameter.value;

        // If search is initiated from the differt route, do not reload the data
        if (wasSearchRoute) {
          $scope.childScope["searchParams" + tableIndex][searchParameter.key] =
            searchParameter.value;
    ...
    $scope.$on(
      ...
      function (event, searchParameter, tableIndex, cacheParam) {
        delete $scope.childScope["searchParams" + tableIndex][
          searchParameter.key
        ];
        delete $rootScope.searchParts[searchParameter.name];
    ...
    $scope.goBack = function () {
      var locationHistory = JSON.parse(
        app.getStorage(app.global.keys.LOCATION_HISTORY)
      );
      var index = 2;
      ...
      var prevUrl =
        locationHistory.length > 0
          ? locationHistory[locationHistory.length - index].Location
          : "/";
      if (locationHistory.length > 0) {
        locationHistory.pop();
        app.setStorage(
          app.global.keys.LOCATION_HISTORY,
      ...
      if (currentIndex != undefined) {
        ...
        while (true) {
          ...
          if ($scope.childScope["searchParams" + searchIndex]) {
            $.each(
              $scope.childScope["searchParams" + searchIndex],
              function (key, value) {
                var is_exist = false;
  ...
  app.post = function (
    ...
    if (
      typeof args !== typeof undefined &&
      typeof args["hideLoading"] !== typeof undefined
    ) {
  ...
  app.getAppData = function () {
    app.post("/b/get_app_data/", {}, false, function (data) {
      ...
      if (
        current_version != "" &&
        sparrow.getStorage("has_release") != "false|" + current_version
      ) {
  ...
  var sendUnreadPushNotification = function (data) {
    ...
    for (var i = 0; i < data.user_notifications.length; i++) {
      ...
      Push.clear();
      Push.close("tag" + [i]);
      user_notification_ids.push(
        data.user_notifications[i]["user_notification_id"]
      );
    }

    if (user_notification_ids.length > 0) {
      app.post(
        "/messaging/update_push_notify/",
        {
          user_notification_ids: JSON.stringify(user_notification_ids),
        },
        false,
        function (data) {}
      );
  ...
  //         $("#notification_count").addClass(
  //           "label-primary notification_indicator classCount"
  //         );
  //         $("#notification_count").text(
  //           data.notification_count + data.user_notification_count
  //         );
  //       } else {
    ...
    events: function () {
      $(document).on("click", ".favourite_icon", function () {
        ...
        if (iSelector.hasClass("glyphicon-star-empty")) {
          ...
          var buttonColor = sparrow.global.get(
            sparrow.global.keys.BUTTON_COLOR
          );

          $(".favourite_page").append(
            '<div class="fav_page page_title' +
              res +
              '" refUrl="' +
              refUrl +
              '" res="' +
              res +
              '"><span class="label-warning" style="background-color:' +
              buttonColor +
              ';width:100px;padding: 3px 0px 2px 5px;border-radius:.25em;"><input id="title' +
              res +
              '" class="editPageName"  type="text" value="' +
              title +
              '" autofocus="autofocus"/></span></div>'
          );
          $("#title" + res).select();
          ...
          sparrow.post(
            "/base/delete_page_favorite/",
            postData,
            false,
            function (data) {
              if (data.favorite_view_id) {
                $("#favoritePage" + data.favorite_view_id).css(
                  "display",
                  "none"
                );
    ...
    postFavData: function (iSelector, res, url, is_besfore_add) {
      ...
      sparrow.post(
        ...
        function (data) {
          ...
          if (data.favorite_view_id) {
            if (is_besfore_add) {
              $(".favourite_page .fav_page:last").before(
                '<div class="fav_page" id = "favoritePage' +
                  data.favorite_view_id +
                  '"> <div class="label label-warning" style="padding:2px 0px 2px 5px;background-color:' +
                  buttonColor +
                  '"><a class="fav-title" style="margin: 3px 20px 0px 5px;" href="' +
                  data.url +
                  '"><span title= "' +
                  postData.name +
                  '">' +
                  postData.name +
                  '</span></a><span refrence= "' +
                  data.favorite_view_id +
                  '" class="icon-cancel-circle favourite-cancel" title="Remove bookmark"></span></div></div>'
              );
            } else {
              $(".favourite_page").append(
                '<div class="fav_page" id = "favoritePage' +
  ...
  app.run(function ($rootScope, $route, $templateCache, $location) {
    ...
    $rootScope.$on("$viewContentLoaded", function () {
      ...
      var suggestion = $templateCache.get(
        "uib/template/typeahead/typeahead-popup.html"
      );
      var matches = $templateCache.get(
      ...
      var gstTax = $templateCache.get("angular-gsttaxs.html");
      var stock_chart = $templateCache.get("angular-wh-loc-picker.html");
      ...
      $templateCache.put("angular-gsttaxs.html", gstTax);
      $templateCache.put("angular-wh-loc-picker.html", stock_chart);
    });
    ...
    $("#top_action_bar").on("click", ".pager-button", function () {
      ...
      if (window.location.href.indexOf("?") > -1) {
        queryString = window.location.href.substring(
      ...
      var locationHistory =
        JSON.parse(app.getStorage(app.global.keys.LOCATION_HISTORY)) || [];
      if (locationHistory.length > 1) {
        var parentPath = locationHistory[locationHistory.length - 2];
        var pathSlash =
          parentPath.Location.substr(-1) != "/"
            ? parentPath.Location + "/"
            : parentPath.Location;
        var pathNonSlash = parentPath.Location.replace("//$/g", "");
        var orgPathSlash =
          parentPath.OrginalPath.substr(-1) != "/"
            ? parentPath.OrginalPath + "/"
            : parentPath.OrginalPath;
        var orgPathNonSlash = parentPath.OrginalPath.replace("//$/g", "");

        var searchCacheParams = app.searchCache.get(pathSlash);
        var searchCacheParams =
          searchCacheParams.length == undefined || searchCacheParams.length == 0
            ? app.searchCache.get(pathNonSlash)
            : searchCacheParams;
        var searchCacheParams =
          searchCacheParams.length == undefined || searchCacheParams.length == 0
            ? app.searchCache.get(orgPathSlash)
            : searchCacheParams;
        var searchCacheParams =
          searchCacheParams.length == undefined || searchCacheParams.length == 0
            ? app.searchCache.get(orgPathNonSlash)
            : searchCacheParams;
        ...
        function (data) {
          if (data.id != undefined && data.id != "") {
            ...
            window.location.hash =
              sparrow.global.get(sparrow.global.keys.PAGER_ROUTE) +
              data.id +
              query;
          }
        }
  ...
  document.addEventListener("keydown", function(event) {
    if (event.key=='Enter'){
      if($('#confirmModal').is(':visible') == true){
  ...
  app.showConfirmDialog = function (
    ModalService,
    msg,
    title,
    callback,
    ...
    var positiveBtnText =
      kwargs != undefined && kwargs.positiveBtnText != undefined
        ? kwargs.positiveBtnText
        : "Yes";
    var negativeBtnText =
      kwargs != undefined && kwargs.negativeBtnText != undefined
        ? kwargs.negativeBtnText
        : "No";
    var hidePositiveBtn =
      kwargs != undefined &&
      kwargs.hidePositiveBtn != undefined &&
      kwargs.hidePositiveBtn == true
        ? "display: none;"
        : "";
    var hideNegativeBtn =
      kwargs != undefined &&
      kwargs.hideNegativeBtn != undefined &&
      kwargs.hideNegativeBtn == true
        ? "display: none;"
        : "";
    ModalService.showModal({
      template:
        ...
        hidePositiveBtn +
        '" ng-click="close(true)" id="yes" class="btn btn-primary" data-dismiss="modal">' +
        positiveBtnText +
        '</button>\
              <button type="button" style="' +
        hideNegativeBtn +
  ...
  app.setDatatable = function (
          ...
          $.grep(Object.keys(listingall), function (k) {
            if (listingall[k].index == tIndex) {
              ...
              if (currentListing.inlineCrud.edit.callback) {
                currentListing.inlineCrud.edit.callback(
                  scope["rowData" + tIndex][id]
                );
              } else if (currentListing.inlineCrud.edit.url) {
                window.location =
                  currentListing.inlineCrud.edit.url +
                  scope["rowData" + tIndex][id].id;
              }
            }
          });
        };
    ...
    scope.openColumnConfigDialog = function (event, tabIndex) {
      var colOrderDataobjs = sparrow.global.get(
        sparrow.global.keys.USER_COL_SETTINGS
      );
      var colOrder = [];
      var checkCol = [];
      for (var b = 0; b < colOrderDataobjs.length; b++) {
        if (
          colOrderDataobjs[b].url ==
            window.location.href.split("/#")[1].replace(/[0-9]/g, "") &&
          colOrderDataobjs[b].table_index == tabIndex
        ) {
      ...
      for (var n = 0; n < listingall.length; n++) {
        currentIndex =
          listingall[n].index == undefined ? 1 : listingall[n].index;
      ...
      for (var q = 0; q < currentColumns.length; q++) {
        if (displayColumns.indexOf(currentColumns[q]) == -1) {
          if (currentColumns[q]["class"] !== "no-display") {
            if (
              currentColumns[q]["coldisplay"] == true ||
              currentColumns[q]["coldisplay"] == undefined
            ) {
              if (currentColumns[q]["name"] !== "") {
      ...
      for (var a = 0; a < displayColumns.length; a++) {
        if (
          displayColumns.length > confingColLength &&
          a > confingColLength - 1
        ) {
          continue;
        }
        if (
          currentColumns[a].class != undefined &&
          currentColumns[a].class.indexOf("hide-items") > -1
        ) {
        ...
        if (
          currentColumns[a].name == "edit" ||
          currentColumns[a].name == null
        ) {
          continue;
        }
        ...
        for (var b = 0; b < checkCol.length; b++) {
          if (
            optionValue == displayColumns[a].name &&
            displayColumns[a].name == checkCol[b]
          ) {
            var checked = "";
          }
        }
        if (currentColumns[a].title == "") {
          var configColumn =
            '<div hidden  class="configColumn" column-name="' +
    ...
    scope.updateSelection = function (index, $event, id) {
      ...
      if (listing.selectionCallback) {
        ...
        for (
          var i = 0;
          i < getDTInstance(index).dataTable.context.rows.length - 1;
          i++
        ) {
    ...
    scope.selectAll = function (index, $event) {
      ...
      if (action == "add") {
        $(getDTInstance(index).dataTable.context.rows).addClass(
          "selected-row-background"
        );
      }

      if (action == "remove") {
        $(getDTInstance(index).dataTable.context.rows).removeClass(
          "selected-row-background"
        );
      }
    ...
    var options = DTOptionsBuilder.newOptions()
      .withOption("fnServerData", function (sSource, aoData, fnCallback) {
        ...
        $.each(
          listing.additionalSearchParams,
          function (i, additionalSearchParam) {
        ...
        if (
          listing.reOrder &&
          $(".reOrder" + listing.uIndex).find(".up_down").length == 0
        ) {
          model_reports = window.location.href
            .split("/#")[1]
            .replace(/[0-9]/g, "")
            .includes("model_reports");
          if (model_reports !== true) {
            var reOrderBtn =
              '<span style="float: left;margin-top: 10px;position:relative;" class="up_down"><i id="col_order" ng-click="openColumnConfigDialog($event,' +
              listing.uIndex +
        ...
        pageNumber =
          app.global.get("SEARCH_PAGING_" + currentRoute.originalPath) || 0;
      ...
      .withOption(
        "dom",
        '<"top"i>rt<"bottom reOrder' + listing.uIndex + '"flip>'
      )
      ...
      .withOption("oLanguage", {
        ...
        sLengthMenu: '<span class="f-footer" style="color:#585858;vertical-align: bottom;display: table-cell;">Show _MENU_ entries</span>',
        sInfo:'<span style="color:#585858;padding: 5px 2px 2px 2px;">Showing _START_ to _END_ of _TOTAL_ entries</span>',
        oPaginate: {
          sNext: '<i class="icon-arrow-2-right" />',
          sPrevious: '<i class="icon-arrow-2-left" />',
        },
      })
      .withOption('lengthMenu', [[10, 25, 50, 100, 200, 300, 400, 500],[10, 25, 50, 100, 200, 300, 400, 500]])
      .withOption("drawCallback", function () {
        ...
        if (scope["searchParams" + listing.index] != undefined) {
          ...
          if (groupColumn != undefined) {
            ...
            api
              .column(groupColumnIndex, { page: "current" })
              .data()
        ...
        rootScope.start = tablePageInfo.start
        rootScope.length = tablePageInfo.length
        // rootScope.order_by = "-id"

        if (
          tablePageInfo.page > 0 &&
          tablePageInfo.end - tablePageInfo.start == 0
        ) {
        ...
        var paginateEle = datatable
          .closest(".dataTables_wrapper")
          .find(".dataTables_paginate");
        var tableLengthEle = datatable
          .closest(".dataTables_wrapper")
        ...
        var currentPageNumber = getDTInstance(
          listing.index
        ).DataTable.page.info().page;
        app.global.set(
          "SEARCH_PAGING_" + currentRoute.originalPath,
          currentPageNumber
        );
      ...
      .withOption("initComplete", function () {
        if (rowGrouping) {
          getDTInstance(listing.index).dataTable.fnSetColumnVis(
            rowGrouping.col,
            false
          );
  ...
  app.getImageTag = function (oldRemark, newRemark) {
    var allElement = $(oldRemark).find("img");
    var aviaElement = $(newRemark).find("img");
    allImageTag = "";
    for (i = 0; i <= allElement.length - 1; i++) {
      allImageTag += allElement[i].outerHTML;
    }
    aviaImageTag = "";
    for (i = 0; i <= aviaElement.length - 1; i++) {
      aviaImageTag += aviaElement[i].outerHTML;
    }
    firstOccurrence = allImageTag.indexOf(aviaImageTag);
    var stringALength = aviaImageTag.length;
    var newString;
    if (firstOccurrence === 0) {
      newString = allImageTag.substring(stringALength);
    } else {
      newString = allImageTag.substring(0, firstOccurrence);
      newString += allImageTag.substring(firstOccurrence + stringALength);
    }
    if (newString != "") {
      sparrow.deleteImage("<p>" + newString + "</p>");
    }
  };

  function searchbarEventsHandler($scope, $route) {
    if ($route.current.$$route.originalPath == "/eda/specifications/") {
      ...
      function (event, searchParameter, tableIndex, cacheParam) {
        ...
        if (searchParameter["type"] == "datePicker") {
          ...
          $scope["searchParams" + tableIndex][
            searchParameter.key + "_from_date"
          ] = para_val[0] + "00:00";
          $scope["searchParams" + tableIndex][
            searchParameter.key + "_to_date"
          ] = para_val[1] + " 23:59";
        }
    ...
    $scope.$on(
      ...
      function (event, tableIndex, cacheParam) {
        if (!$scope["searchParams" + tableIndex]) {
          return;
        }
        $scope["searchParams" + tableIndex] = {};
  ...
  app.setup = function (
    ...
    if (topActionbar) {
      ...
      if (topActionbar.delete) {
        listingAction.selections.push({
          ...
          multiselect:
            topActionbar.delete.multiselect === undefined
              ? true
              : topActionbar.delete.multiselect,
        });
      }
      ...
      if (topActionbar.add) {
        $rootScope.onAdd = function (event) {
          window.location =
            ...
            "0" +
            (topActionbar.add.filters != undefined &&
            topActionbar.add.filters != ""
              ? "?" + topActionbar.add.filters
              : "") +
            "/?&backhis=true";
        };
          ...
          if ($scope["selected1"][0] == undefined) {
            ...
            app.redirect(
              topActionbar.edit.url +
                $scope["selected1"][0] +
                (topActionbar.edit.filters != undefined &&
                topActionbar.edit.filters != ""
                  ? "?" + topActionbar.edit.filters
                  : "")
            );
          }
    ...
    if (allListing) {
      ...
      var colOrderData =
        sparrow.global.get(sparrow.global.keys.USER_COL_SETTINGS) || [];

      var uiSettingsObj = {};
      for (var x = 0; x < colOrderData.length; x++) {
        uiSettingsObj[colOrderData[x].table_index + "-" + colOrderData[x].url] =
          JSON.parse(colOrderData[x].col_settings);
      }
      ...
      for (var j = 0; j < allListing.length; j++) {
        ...
        for (var k = 0; k < allListing[j].columns.length; k++) {
          if (allListing[j].columns[k].class !== "no-display") {
            if (
              allListing[j].columns[k].coldisplay == true ||
              allListing[j].columns[k].coldisplay == undefined
            ) {
              orginalColumn[j].push(allListing[j].columns[k].name);
              originalCol[j][allListing[j].columns[k].name] =
                allListing[j].columns[k];
            }
          }
        }
      }
      var statUrl = window.location.href.split("/#")[1].replace(/[0-9]/g, "");
      update_urls = Object.keys(uiSettingsObj);
      datas = $route.current.originalPath;
      urls_data = {};
      var statUrl_data = "";
      for (i = 0; i < statUrl.length; i++) {
        if (statUrl[i] == "?") {
          break;
        }
        statUrl_data = statUrl_data + statUrl[i];
      }
      for (var i = 0; i < update_urls.length; i++) {
        url_bool = update_urls[i].includes("-" + statUrl_data);
        if (url_bool == true) {
          update_urls_index = update_urls[i][0];
          urls_data[update_urls_index] = update_urls[i].replace(/[0-9]/g, "");
        }
      }
      ...
      for (var i = 0; i < allListing.length; i++) {
        ...
        if (colOrderData.length > 0) {
          ...
          var table_index =
            allListing[i].index != undefined ? allListing[i].index : 1;
          if (
            window.location.href
              .split("/#")[1]
              .replace(/[0-9]/g, "")
              .includes("model_reports") !== true
          ) {
            if (urls_data.length != 0) {
              if (
                uiSettingsObj.hasOwnProperty(
                  table_index + urls_data[table_index]
                )
              ) {
                var dbCol = uiSettingsObj[table_index + urls_data[table_index]];
                ...
                for (var c = 0; c < hide_col.length; c++) {
                  if (jQuery.inArray(hide_col[c], defaultColumns) !== -1) {
                    if (
                      originalDisplayCol[userColumns.indexOf(hide_col[c])][
                        "class"
                      ] == undefined
                    ) {
                      originalDisplayCol[userColumns.indexOf(hide_col[c])][
        ...
        if (listing.search && listing.index) {
          ...
          for (var j = 0; j < listing.search.params.length; j++) {
            $scope["availableSearchParams" + listing.index].push(
              listing.search.params[j]
            );
          }
          ...
          for (var j = 0; j < listing.search.params.length; j++) {
            if ("default_val" in listing.search.params[j]) {
              $scope["searchParams" + listing.index][
                listing.search.params[j]["key"]
              ] = listing.search.params[j]["default_val"];
            }
          }
          ...
          /*
                    ...
                    */
          var searchCacheParams = app.searchCache.get(
            $route.current.originalPath
          );
        ...
        var listingObj = {
          ...
          displayLength:
            listing.displayLength ||
            app.global.get(app.global.keys.DISPLAY_ROW) ||
            10,
          ...
          rowGrouping:
            listing.rowGrouping == undefined ? false : listing.rowGrouping,
          emptyTblMsg: listing.emptyTblMsg || "No data available",
        ...
        // BIND DATATABLE
        $scope["dtOptions" + tIndex] = app.setDatatable(
          onBind,
          $compile,
          DTOptionsBuilder,
          $scope,
        ...
        if (idColumn === undefined) {
          console.log(
            "Id column is not defined. Make sure you have not supplied id columns otherwise data columns mismatched and error occurrs in data binding."
          );
        }
        ...
        if (listing.srNumber) {
          var srNumberColumn = DTColumnBuilder.newColumn(null)
            .withTitle("Sr No")
            .withClass("sr-number")
        ...
        for (var k = 0; k < listing.columns.length; k++) {
          ...
          var column = DTColumnBuilder.newColumn(listColumn.name).withOption(
            "type",
            "num"
          );
          ...
          if (listColumn.link) {
            ...
            column.renderWith(function (data, type, full, meta) {
              ...
              var dialog =
                colLinkData.dialog == undefined ? false : colLinkData.dialog;
              var iframeTitle = colDataEdit.title;
              var linkTitleKey =
                "title" in colLinkData.params
                  ? colLinkData.params["title"]
                  : undefined;
              // Clone object
    ...
    if (
      $(".details-footer").length != 0 &&
      $("ang-subscriptions").length != 0
    ) {
      ...
      for (i = 0; i < allListing.length; i++) {
        ...
        if (listing.inlineCrud) {
          ...
          function actionsHtml(data, type, full, meta) {
            ...
            if (isDelete) {
              template +=
                '<div class="pull-right"><i class="icon-trash list-btn" ng-click="inlineDelete(' +
                instanceId +
                "," +
                data.id +
                ')"></i>&nbsp;';
          ...
          $scope["dtColumns" + tIndex].push(
            DTColumnBuilder.newColumn(null)
              .withTitle('<span style="padding-right:65px;"></span>')
    ...
    $("#app_container").on("mouseover", ".edit-link", function () {
      var id =
        $(this).children().attr("name") + "_" + $(this).children().attr("id");
      ...
      var colData =
        $scope[
          "colDataEdit" +
            ...
            $(this).children().attr("col")
        ];
      var id =
        $(this).children().attr("name") + "_" + $(this).children().attr("id");
      var rowData =
        $scope["rowData" + $(this).children().attr("instance")][
  ...
  app.showAttachments = function (ModalService, objectId, appName, modelName) {
    ModalService.showModal({
      ...
      $(".modal-backdrop").each(function (i, obj) {
        if (!$(obj).hasClass("fade")) $(obj).remove();
      });
      modal.close.then(function (result) {});
    });
  };

  app.showImportDialog = function (
    ModalService,
    modelName,
    title,
    postUrl,
    ...
    ModalService.showModal({
      ...
      if (modelName == "part_offers") {
        setAutoLookup("supplier_lookup", "/b/lookups/supplier/", "", false);
        ms = $("#supplier_lookup").magicSuggest();
        supplier_name = ms.getSelection()[0];
      }
  ...
  app.showTrackingInDialog = function (
    ModalService,
    title,
    url,
    modelName,
    ...
    ModalService.showModal({
      templateUrl:
        "/inventory/tracking_in/" + modelName + "/" + id + "/" + qty + "/",
          ...
          function (data) {
            if (data.code == 1) {
              ...
              $("input.serial-input").each(function () {
                ...
                var new_serial =
                  prefix +
                  ("0000000000" + strSerial).substring(strSerial.length);
                $(this).val(new_serial);
                serial = serial + 1;
              });
            }
  ...
  app.showTrackingOutDialog = function (
    ModalService,
    title,
    url,
    modelName,
    ...
    $("#loading-image").show();
    customer_id = uploadData.customer != null ? uploadData.customer : 0;
    ModalService.showModal({
      templateUrl:
        "/inventory/tracking_out/" +
        modelName +
        "/" +
        id +
        "/" +
      ...
      modal.element.on("shown.bs.modal", function () {
          ...
          if ($("div.product-name h4").length > 0) {
            $("div.product-name h4").each(function () {
              ...
              if ($.isNumeric(requiredQty) && $.isNumeric(requiredQty) > 0) {
                ...
                var lots = $(
                  'label.stock-qty[data-product-id="' +
                    productId +
                    '"][data-line-id="' +
                    lineId +
                    '"]'
                ...
                $(lots).each(function () {
                  ...
                  var lotQty = $(
                    'label.stock-qty[data-id="' +
                      lotId +
                      '"][data-line-id="' +
                      lineId +
                      '"]'
                  ).text();
                  var serial_lot_number = $(
                    'label.serial-num[data-id="' +
                      lotId +
                      '"][data-line-id="' +
                      lineId +
                      '"]'
        ...
        if ($("div.product-name h4").length > 0) {
          ...
          var duplicateWareHouseLocId = $("#id_duplicate_part_loc").attr(
            "value"
          ); // Remove warehouse loc EC specific code
          $("div.product-name h4").each(function () {
            var productId = $(this).attr("data-product-id");
            // var customerWarehouse = $(this).attr('customer-warehouse');
            var lineId = $(this).attr("data-line-id");
            var requiredQty = $(
              'div[data-product-id="' +
                productId +
            ...
            var isGenericGroup =
              $(this).attr("generic-group") == "true" ? true : false;
            ...
            if ($.isNumeric(requiredQty) && $.isNumeric(requiredQty) > 0) {
              ...
              $(lots).each(function () {
                ...
                var reservedTrackingStr = $("#reservedTrackingStr").attr(
                  "refrence"
                );
                ...
                if (
                  ...
                  if (modelList.indexOf(modelName) >= 0) {
                    if (requiredQty > 0) {
                      if (
                        modelName == "mfg_order" &&
                        $("#id_is_sale_order").attr("value") != ""
                      ) {
                        var customerWarehouse = $(
                          "#row" + lotId + '[data-line-id="' + lineId + '"]'
                        ).attr("customer-warehouse");
                        ...
                        if (
                          ...
                          $("#row" + lotId).addClass("match-lot show-style");
                          $(
                            'input.lot-number[data-id="' +
                              lotId +
                              '"][data-line-id="' +
                        ...
                        if (
                          ...
                          $(".serialLotRows" + productId + " > .row").addClass(
                            "match-lot"
                          );
                          $(
                            'input.lot-number[data-id="' +
                        ...
                        if (isGenericGroup) {
                          ...
                          $(
                            'label.warehouse-location[data-id="' +
                              lotId +
                              '"][data-line-id="' +
                              lineId +
                              '"]'
                          ).attr("value") ==
                            $("#id_parts_location").attr("value") ||
                          $(
                            'label.warehouse-location[data-id="' +
                              lotId +
                              '"][data-line-id="' +
                              lineId +
                              '"]'
                          ).attr("value") == duplicateWareHouseLocId
                        ) {
                          if (
                            $("#row" + lotId).attr("warehouse-location") != ""
                          ) {
                            $(
                              'input.lot-number[data-id="' +
                                lotId +
                  ...
                  // Select already reserved tracking numbers.
                    ...
                    var result =
                      reservedTrackingNumbers.includes(serial_lot_number);
                    if (result) {
                      $(
                        'input.lot-number[data-id="' +
                          lotId +
            ...
            if (
              ...
              $('div.row[id="product' + productId + '"]').addClass(
                "match-product"
              );
              $('div.row[id="border' + productId + '"]').addClass(
                "match-product"
              );
            }
          });
  ...
  app.showSelectLotDialog = function (
    ModalService,
    title,
    url,
    modelName,
    ...
    qty = qty != null ? qty : 0;
    customer_id = data.customer != null ? data.customer : 0;
    ModalService.showModal({
      templateUrl:
        "/inventory/select_lot/" +
        modelName +
        "/" +
        id +
        "/" +
  ...
  app.getQueryStringParams = function (sParam) {
    var sPageURL = "";
    if (window.location.href.indexOf("?") > -1) {
      sPageURL = window.location.href.slice(
        window.location.href.indexOf("?") + 1
      );
    }
  ...
  var openIframe = function (
    ...
    var iframeHTML =
      '<div id="' +
      iframeModelId +
      '"  class="modal fade frame-dialog" tabindex="-1" role="dialog" data-keyboard="false" style="width:64%; margin-left:18%;" data-backdrop="static">\
                          <div class="modal-dialog modal-lg" role="document">\
                              <div class="modal-content">\
                                  <div class="modal-header">\
                                      <button type="button" id="' +
      iframeCloseId +
      '"  class="close closeiframe" data-dismiss="modal" aria-label="Close">\
    ...
    if (ttlOpenWindow > 0) {
      title =
        parent.$("#iframe_title" + (parseInt(ttlOpenWindow) - 1)).html() +
    ...
    parent
      .$("#" + iframeId)
      .css("height", parent.document.body.clientHeight - 170 + "px");
  ...
  app.setIframeTitle = function (title) {
    if (app.inIframe() && parent.globalIndex.iframeCloseCallback.length > 0) {
      var iFrameCloseCallback =
        parent.globalIndex.iframeCloseCallback[
          parent.globalIndex.iframeCloseCallback.length - 1
        ];
  ...
  app.applyEditMode = function (containerId, firstElementId) {
    var preDisabledInputs = $(
      containerId +
        ' input[default-disabled="true"],select[default-disabled="true"]'
    );
    ...
    $(containerId + " .has-error .form-control").css(
      "cssText",
  ...
  app.applyReadOnlyMode = function (containerId) {
    var preDisabledInputs = $(
      containerId +
        ' input:disabled,input[readonly="readonly"],select:disabled,select[readonly="readonly"]'
    );
    var inputs = $(containerId + " :input").not(
      containerId + " :input[disable-false]"
    );
    var preDisabledInputIds = [];
    $.each(preDisabledInputs, function () {
      preDisabledInputIds.push(this.id);
    });
    var preDisabledLabels = $(
      containerId +
        ' label[for="' +
        preDisabledInputIds.join('"],[for="') +
        '"]'
    );
    ...
    $("#app_container").on(
      "click",
      ".read-only-mode , .read-only-labels",
      function () {
  ...
  app.setControlFocus = function (firstElementId) {
    setTimeout(function () {
      if (
        !(
          $(firstElementId).hasClass("ms-ctn") ||
          $(firstElementId).parent().hasClass("input-group")
        )
      ) {
  ...
  app.pushLocationHistory = function (originalPath, tabUrl) {
    var locationHistory =
      JSON.parse(sparrow.getStorage(sparrow.global.keys.LOCATION_HISTORY)) ||
      [];
    ...
    sparrow.setStorage(
      sparrow.global.keys.LOCATION_HISTORY,
      JSON.stringify(locationHistory)
    );
  };
  ...
  app.getEntityIframeURL = function (
    appName,
    modelName,
    entityId,
    related_to,
  ...
  app.getFileIcon = function (extension) {
    var isImage = sparrow.isImage(extension);

    if (isImage) {
      return "/static/base/images/file-icons/image.svg";
    } else {
      switch (extension) {
        case "doc":
          return "/static/base/images/file-icons/word.svg";
        case "docx":
          return "/static/base/images/file-icons/word.svg";
        case "pdf":
          return "/static/base/images/file-icons/pdf.svg";
        case "ppt":
          return "/static/base/images/file-icons/powerpoint.svg";
        case "pptx":
          return "/static/base/images/file-icons/powerpoint.svg";
        case "xlsx":
          return "/static/base/images/file-icons/excel.svg";
        case "xls":
          return "/static/base/images/file-icons/excel.svg";
        case "csv":
          return "/static/base/images/file-icons/csv.svg";
        case "xml":
          return "/static/base/images/file-icons/xml.svg";
        case "txt":
          return "/static/base/images/file-icons/txt.svg";
        case "zip":
          return "/static/base/images/file-icons/zip.svg";
        case "rar":
          return "/static/base/images/file-icons/rar.svg";
        default:
          return "/static/base/images/file-icons/file.svg";
      }
    }
  };

  app.validatePriceDecimalInput = function (value, decimal_place, id) {
    if (value.split(".").length == 2) {
      if (parseFloat(value.split(".")[1].length) > decimal_place) {
        value =
          value.split(".")[0] +
          "." +
          parseFloat(value.split(".")[1].slice(0, decimal_place));
        $("#" + id).val(value);
      }
    }
  };

  app.uploadImage = function (image, summernote_id) {
    $("#loading-image").show();
    var data = new FormData();
    data.append("image", image);

    $.ajax({
      url: "/base/upload_wysiwyg_media/",
      cache: false,
      contentType: false,
      processData: false,
      data: data,
      ...
      success: function (data) {
        if (data.code == 1) {
          ...
          $("#loading-image").hide();
          app.showMessage("appMsg", app.MsgType.Error, data.msg, 10);
        }
      },
      error: function (data) {
        app.showMessage("appMsg", app.MsgType.Error, "Error occurred", 10);
  ...
  app.deleteImage = function (tag) {
    if (jQuery.parseHTML(tag) != null) {
      var imgExists = tag.indexOf('<img src="');
      if (imgExists > -1) {
        var imageElement = $(tag).find("img");
        for (i = 0; i <= imageElement.length - 1; i++) {
          imageTag = imageElement[i].outerHTML;
          imagePath = $(imageTag).attr("src");
          data = { image: imagePath };
          $.ajax({
            url: "/base/delete_wysiwyg_media/",
            data: data,
            type: "POST",
            success: function (data) {},
            error: function (data) {},
  ...
  app.getImageTag = function (oldRemark, newRemark) {
    ...
    if (newString != "") {
      if (!app.deletedImagesStr.includes(newString)) {
        app.deletedImagesStr.push(newString);
      }
      sparrow.deleteImage("<p>" + newString + "</p>");
    }
  };
  ...
  // if (!app.inIframe()) {
  //     // Do not subscribe to pusher in the development environment.
  //     if (window.location.href.indexOf("localhost") == -1 && window.location.href.indexOf("192.168") == -1) {
  //         app.pusher = new Pusher('909e80f447f2a23d855b', {
  //             cluster: 'eu',
  //             forceTLS: true,
  //             authEndpoint: '/chat/client/auth/',
  //         });
  //         app.pusher.subscribe("presence-sparrow");
  //     }
  // }
  return app;
}
