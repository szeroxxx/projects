...
def task(request):
    try:
        if request.method == "POST":
            ...
            show_private = False
            operators = (
                Operator.objects.filter(is_deleted=False,is_active=True)
                .values("operator_group", "id", "user__username", "sub_group_of_operator__sub_group_name", "sub_group_of_operator")
                .order_by("sub_group_of_operator__sub_group_name")
            )
            group_b = []
            group_fee = []
            customer = []
            ...
            group_fee_sub_group = []
            customer_sub_group = []
            backoffice_and_oth_sub_group = []
            no_group_sub_group = []
            
            group_b_sub_group_duplicate = []
            group_fee_sub_group_duplicate = []
            customer_sub_group_duplicate = []
            backoffice_and_oth_sub_group_duplicate = []
            no_group_sub_group_duplicate = []
            for user in operators:
                if user["operator_group"] == "GROUP_B":
                    group_b_ids.append(user["id"])
                    if user["sub_group_of_operator__sub_group_name"] not in group_b_sub_group_duplicate:
                        group_b_sub_group_duplicate.append(user["sub_group_of_operator__sub_group_name"])
                        group_b_sub_group.append(
                            {
                                "sub_group_name":  user["sub_group_of_operator__sub_group_name"],
                                "sub_group_id":  user["sub_group_of_operator"] if user["sub_group_of_operator"] else 0,
                            }
                        )
                    group_b.append(
                        {
                            ...
                            "sub_group": user["sub_group_of_operator__sub_group_name"]
                        }
                    )
                if user["operator_group"] == "GROUP_FEE":
                    group_fee_ids.append(user["id"])
                    if user["sub_group_of_operator__sub_group_name"] not in group_fee_sub_group_duplicate:
                        group_fee_sub_group_duplicate.append(user["sub_group_of_operator__sub_group_name"])
                        group_fee_sub_group.append(
                            {
                                "sub_group_name":  user["sub_group_of_operator__sub_group_name"],
                                "sub_group_id":  user["sub_group_of_operator"] if user["sub_group_of_operator"] else 0,
                            }
                ...
                if user["operator_group"] == "CUSTOMER":
                    ...
                    if user["sub_group_of_operator__sub_group_name"] not in customer_sub_group_duplicate:
                        customer_sub_group_duplicate.append(user["sub_group_of_operator__sub_group_name"])
                        customer_sub_group.append(
                            {
                                "sub_group_name":  user["sub_group_of_operator__sub_group_name"],
                                "sub_group_id":  user["sub_group_of_operator"] if user["sub_group_of_operator"] else 0,
                            }
                ...
                if user["operator_group"] == "BACKOFFICE_AND_OTH":
                    ...
                    if user["sub_group_of_operator__sub_group_name"] not in backoffice_and_oth_sub_group_duplicate:
                        backoffice_and_oth_sub_group_duplicate.append(user["sub_group_of_operator__sub_group_name"])
                        backoffice_and_oth_sub_group.append(
                            {
                                "sub_group_name":  user["sub_group_of_operator__sub_group_name"],
                                "sub_group_id":  user["sub_group_of_operator"] if user["sub_group_of_operator"] else 0,
                            }
                ...
                if user["operator_group"] == None:
                    ...
                    if user["sub_group_of_operator__sub_group_name"] not in no_group_sub_group_duplicate:
                        no_group_sub_group_duplicate.append(user["sub_group_of_operator__sub_group_name"])
                        no_group_sub_group.append(
                            {
                                "sub_group_name":  user["sub_group_of_operator__sub_group_name"],
                                "sub_group_id":  user["sub_group_of_operator"] if user["sub_group_of_operator"] else 0,
                            }
            ...
            select_oper_sub_group_b = []
            select_oper_sub_group_fee = []
            select_oper_sub_customer = []
            select_oper_sub_backoffice = []
            select_oper_sub_no_group = []
            if task_id is not None and task_id != "0":
                ...
                for chk_operator in operator_.split(","):
                    for operator in operators:
                        if operator["operator_group"] == "GROUP_B":
                            if int(operator["id"]) == int(chk_operator):
                                if operator["sub_group_of_operator__sub_group_name"] not in select_oper_sub_group_b:
                                    select_oper_sub_group_b.append(operator["sub_group_of_operator__sub_group_name"])
                        if operator["operator_group"] == "GROUP_FEE":
                            if int(operator["id"]) == int(chk_operator):
                                if operator["sub_group_of_operator__sub_group_name"] not in select_oper_sub_group_fee:
                                    select_oper_sub_group_fee.append(operator["sub_group_of_operator__sub_group_name"])
                        if operator["operator_group"] == "CUSTOMER":
                            if int(operator["id"]) == int(chk_operator):
                                if operator["sub_group_of_operator__sub_group_name"] not in select_oper_sub_customer:
                                    select_oper_sub_customer.append(operator["sub_group_of_operator__sub_group_name"])
                        if operator["operator_group"] == "BACKOFFICE_AND_OTH":
                            if int(operator["id"]) == int(chk_operator):
                                if operator["sub_group_of_operator__sub_group_name"] not in select_oper_sub_backoffice:
                                    select_oper_sub_backoffice.append(operator["sub_group_of_operator__sub_group_name"])
                        if operator["operator_group"] == None:
                            if int(operator["id"]) == int(chk_operator):
                                if operator["sub_group_of_operator__sub_group_name"] not in select_oper_sub_no_group:
                                    select_oper_sub_no_group.append(operator["sub_group_of_operator__sub_group_name"])
                ...
                return HttpResponse(
                    render_to_string(
                        "task/task.html",
                        {
                            ...
                            "no_group":no_group,
                            "group_b_sub_group" : group_b_sub_group,
                            "group_fee_sub_group" : group_fee_sub_group,
                            "customer_sub_group" : customer_sub_group,
                            "backoffice_and_oth_sub_group" : backoffice_and_oth_sub_group,
                            "no_group_sub_group" : no_group_sub_group,
                            "select_oper_sub_group_b": select_oper_sub_group_b,
                            "select_oper_sub_group_fee": select_oper_sub_group_fee,
                            "select_oper_sub_customer": select_oper_sub_customer,
                            "select_oper_sub_backoffice": select_oper_sub_backoffice,
                            "select_oper_sub_no_group": select_oper_sub_no_group,
                        },
                    )
                )
            else:
