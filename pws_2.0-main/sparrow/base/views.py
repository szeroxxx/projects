...
from base.services import ReleaseNoteOperation
...
from pws.models import Company, Operator, Order, CompanyUser, TechnicalHelp
# from sqlalchemy.sql import text
from stronghold.decorators import public
import base64
from pws.models import UserEfficiencyLog, NonConformityDetail
from django.db.models import F, Sum, Value, Func, Count
...
def index(request):
    ...
    for date_ in date_list:
        if date_.weekday() == 6:
            sundays += 1
    total_days = int(len(date_list)) - int(sundays)
    query.add(Q(operator__user=request.user), query.connector)
    query.add(Q(created_on__month=today.month), query.connector)
    query.add(Q(created_on__year=today.year), query.connector)
    user_efficiency_logs = (
        ...
        .annotate(
            operator_name=Func(F('operator__user__first_name'), Value(' '), F('operator__user__last_name'), function='CONCAT'),
            month_average_efficiency=Sum("total_work_efficiency"),
            today_efficiency=Sum("total_work_efficiency", filter=Q(created_on__day=today.day))
        )
    )
    query1 = Q()
    query1.add(Q(operator__user=request.user), query1.connector)
    query1.add(Q(nc_detail_date__month=today.month), query1.connector)
    query1.add(Q(nc_detail_date__year=today.year), query1.connector)
    nc_counts = (
        NonConformityDetail.objects.filter(query1)
        .values(
            "operator"
        )
        .annotate(
            month_nc_count=Count("id")
        )
    )
    ...
    message_ = Message.objects.filter((Q(task_id__due_date__gte=datetime.datetime.now()) | Q(task_id__due_date__isnull=True)), task_id__private=False, operator_id__user__username=username, is_delete=False, is_read=False, task_id__general=False).values()
    message = Message.objects.filter((Q(task_id__due_date__gte=datetime.datetime.now()) | Q(task_id__due_date__isnull=True)), task_id__private=False, operator_id__user__username=username, is_delete=False, is_read=False).values("id").count()
    ...
    company = Company.objects.prefetch_related("companyuser_set").filter(companyuser__user__username=username).values("company_img").first()
    ...
    if check_expiration.para_value == "True":
        ...
        if currency_rates:
            return render(
                ...
                {"currencyrates": currency_rates, "company_img": company.get("company_img"), "base_currency": request.session["base_currency"]},
            )
    ...
    if user_role:
        if user_role["group__name"] in ["Leader", "Admin", "Group Leader"]:
            is_leader = True
    is_tech_help = TechnicalHelp.objects.filter(status="attend").values("created_by").first()
    is_tech_help_ = False
    if is_tech_help:
        is_tech_help_ = True
    tech_status = TechnicalHelp.objects.filter(created_by_id=request.user.id).values("status").last()
    tech_help = False
    if tech_status:
        if tech_status["status"] == "attend":
            tech_help = True
    query = Q()
    if not is_leader:
        query.add(Q(created_by=request.user.id), query.connector)
    tech_help_user = TechnicalHelp.objects.filter(query, status="attend").values("id", "created_by__username", "created_on")
    company_user = CompanyUser.objects.filter(user_id=request.user.id)
    ...
    if user_efficiency_logs:
        for user_efficiency_log in user_efficiency_logs:
            context["operator_name"] = user_efficiency_log["operator_name"]
            context["month_average_efficiency"] = round(((user_efficiency_log["month_average_efficiency"] / 450) * 100) / total_days, 2) if user_efficiency_log["month_average_efficiency"] else 0.0
            context["today_efficiency"] = round((user_efficiency_log["today_efficiency"] / 450) * 100, 2) if user_efficiency_log["today_efficiency"] else 0.0
    else:
        context["operator_name"] = user_full_name
        context["month_average_efficiency"] = 0.0
        context["today_efficiency"] = 0.0
    if nc_counts:
        for nc_count in nc_counts:
            context["month_nc_count"] = nc_count["month_nc_count"] if nc_count["month_nc_count"] else 0
    else:
        context["month_nc_count"] = 0
    ...
    # context["db_bg_color"] = theme_info["db_bg_color"]
    ...
    context["company_img"] = company.get("company_img") if company else None
    ...
    context["general"] = False if len(message_) == 0 else True
    context["permissions"] = permissions
    context["user_email"] = user_email
    context["CLICKUPAPI"] = project_settings.CLICK_UP_API
    context["tech_help"] = tech_help
    context["is_leader"] = is_leader
    context["is_tech_help_"] = is_tech_help_
    context["tech_help_user"] = tech_help_user
...
def create_remark(app_name, model_name, entity_id, remark, remark_type, created_by_id, model_remark_field, remark_type_id, source_info_id, scope_id, prep_by=None, prep_on=None):
    try:
        ...
        prep_section = None
        if app_name == "pws" and model_name == "order":
