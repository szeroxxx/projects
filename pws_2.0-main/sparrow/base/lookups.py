...
from pws.models import (Company, CompanyUser, Layer,
...
def lookups(request, model):
    ...
    if model == "processes":
        process_names = OrderProcess.objects.filter(~Q(code__in=["exception", "back_to_previous", "ppa_exception"]), name__icontains=q).order_by("id").values("name", "id")
        response = []
    ...
    if model == "processes_allocation":
        process_names = OrderProcess.objects.filter(~Q(code__in=["panel", "upload_panel", "exception", "back_to_previous", "ppa_exception"]), name__icontains=q).order_by("id").values("name", "id")
        response = []
        for process_name in process_names:
            response.append({"name": process_name["name"], "id": process_name["id"]})
        if selectedIds and selectedIds != "":
