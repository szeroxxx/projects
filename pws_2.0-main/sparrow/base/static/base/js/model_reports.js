...
function ReportsInit() {
    ...
    sparrow.registerCtrl('modelReportsCtrl', function ($scope, $rootScope, $route, $routeParams, $compile, DTOptionsBuilder, DTColumnBuilder, $templateCache, $location) {
        ...
        pageTitle= 'Reports' + ' - ' + capitalizeFirstLetter($routeParams.type)
        if ($routeParams.type == 'eda'){
            pageTitle = 'Reports' + ' - ' + 'EDA'
        }
        config = {
            pageTitle: pageTitle,
            listing: [
                {
                    index: 1,
                    search: {
                        params: [{ key: 'report__title__icontains', name: 'Report' }],
                    },
                    ...
                    columns: [
                        {
                            ...
                            renderWith: function (data, type, full, meta) {
                                ...
                                return (
                                    '<i class="' +
                                    glyphiconStar +
                                    '" style="color:rgb(121, 125, 131);cursor:pointer;' +
                                    bookmarkColor +
                                    '" title="' +
                                    favreportTitle +
                                    '" id = "bookmark' +
