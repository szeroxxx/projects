function preparationsInit(data) {
  sparrow.registerCtrl(
    ...
    function (
      ...
      $scope.allPageTitles = {
            ...
            BOM_incoming: 'Preparation (BOM incoming)',
            SI: 'Preparation (SI)',
            SICC: 'Preparation (SI CC)',
            BOM_CC: 'Preparation (BOM CC)',
            FQC: 'Preparation (FQC)',
        };
      ...
      $scope.allIndex = {
          ...
          listing: [
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/analysis/0/" +
                $scope.requestStatus["analysis"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
            },
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/incoming/0/" +
                $scope.requestStatus["incoming"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
            },
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/BOM_incoming/0/" +
                $scope.requestStatus["BOM_incoming"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
            },
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/SI/0/" +
                $scope.requestStatus["SI"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
            },
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/SICC/0/" +
                $scope.requestStatus["SICC"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
            },
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/BOM_CC/0/" +
                $scope.requestStatus["BOM_CC"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
            },
            {
              ...
              search: searchObj,
              url:
                "/pws/workspace_search/FQC/0/" +
                $scope.requestStatus["FQC"] +
                "/",
              crud: true,
              scrollBody: true,
              columns: columnObj,
        ...
        function onExport() {
          if (data.permissions["can_export_job_processing_preparation"] == false) {
          ...
          sparrow.downloadData("/pws/exports_job_processing/", {
      ...
      $scope.onTabChange = function (status, index) {
          ...
          history.replaceState(undefined, undefined, '#/job_processing/preparation/?state=' + status);
          sparrow.pushLocationHistory($route.current.originalPath, '#/job_processing/preparation/?state=' + status);
          if ($scope.requestStatus[status] == false) {
              config.listing[index - 1].url = '/pws/workspace_search/' + status + '/0/true/';
      ...
      function OnReserve() {
        $scope.ReserveorderList = [];
            if (data.permissions['reserve_order_preparation'] == false) {
                sparrow.showMessage('appMsg', sparrow.MsgType.Error, 'You do not have permission to perform this action', 3);
                return;
            }
     ...
     function OnRelease() {
        if (data.permissions['release_order_preparation'] == false) {
                sparrow.showMessage('appMsg', sparrow.MsgType.Error, 'You do not have permission to perform this action', 3);
                return;
        }
        var selectedId = $scope.getSelectedIds($scope.tabIndex);
        ...
        sparrow.showConfirmDialog(ModalService, "Are you sure you want to release operator?", "Release operator",
            function (confirm) {
                if (confirm) {
                    sparrow.post(
                    "/pws/release_operator/" + selectedId + "/",
                    {},
                    false,
                    function (data) {
                        if (data.code == 1) {
      ...
      function onSendtonext() {
        if (data.permissions['send_to_next_preparation'] == false) {
            sparrow.showMessage('appMsg', sparrow.MsgType.Error, 'You do not have permission to perform this action', 3);
            return;
        }
        ...
        sparrow.post(
            '/pws/send_to_next_details/' + orderId + '/',
            {},
            false,
            function (data) {
    ...
    function onBackToprevious() {
        ...
        sparrow.post(
          '/pws/back_to_previous_details/' + orderId + '/',
          {},
          false,
          function (data) {
    ...
    function showGenerateException(scope){
        ...
        if (data.permissions['generate_exception_preparation'] == false) {
            sparrow.showMessage('appMsg', sparrow.MsgType.Error, 'You do not have permission to perform this action', 3);
            return;
        }
        else{
        ...
        function onFileSearch() {
            if (data.permissions['view_file_preparation'] == false) {
                sparrow.showMessage('appMsg', sparrow.MsgType.Error, 'You do not have permission to perform this action', 3);
                return;
            }
