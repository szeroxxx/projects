function operatorInit(data) {
  sparrow.registerCtrl(
    ...
    function (
      ...
      $scope.editSubGropOperator = function (event, sub_group_id) {
        event.stopPropagation();
        sparrow.setControlFocus("#id_sub_group" + sub_group_id);
        $(".edit-sub-group").css("display", "flex");
        $("#id_sub_group" + sub_group_id).show()
        $("#sub_group_save" + sub_group_id).show()
        $("#sub_group_cancel" + sub_group_id).show()
        $(".mouseover").hide()
      };

      $scope.cancelSaveSubGropOperator = function (event, sub_group_id) {
        $("#id_sub_group" + sub_group_id).hide()
        $("#sub_group_save" + sub_group_id).hide()
        $("#sub_group_cancel" + sub_group_id).hide()
        $(".mouseover").show()
      };

      $scope.saveSubGropOperator = function (event, sub_group_id) {
        event.stopPropagation();
        var sub_group_name = $("#id_sub_group" + sub_group_id).val()
        sparrow.post(
          "/pws/edit_sub_group_of_operator/",
          {
      ...
      $scope.removeSubGropOperator = function (event, sub_group_id) {
        event.stopPropagation();
        sparrow.showConfirmDialog(ModalService, 'Are you sure, want to remove sub group?<br>Deleted sub group will be removed from all the operator who has this sub group.', 'Delete sub group', function (confirm) {
          if (confirm) {
            sparrow.post(
              "/pws/delete_sub_group_of_operator/",
              {
                sub_group_id: sub_group_id,
              },
              false,
      ...
      $("#myInput").keydown(function(e) {
        ...
        $("#id_sub_group").removeAttr('style');
          ...
          sparrow.post(
            "/pws/create_sub_group_of_operator/",
            {sub_group: sub_group},
            false,
            function (data) {
              if (data.code == 1) {
          ...
          if (
              ...
              if(password.match(/[a-z]/g) &&
                ...
                sparrow.postForm(
                  ...
                  function (data) {
                  if (data.code == 1) {
                      window.location.hash = "#/pws/operators/" ;
                      $route.reload();
                      }
                  }
                );
